...
//    ftps        990/udp    ftp protocol, control, over TLS/SSL
    ...
    * @since 3.1
    */
    // https://stackoverflow.com/questions/58863497/ftpsclient-throw-exception-javax-net-ssl-sslhandshakeexception-remote-host-clos
       ...
       if (socket instanceof SSLSocket) {
          ...
          if (session.isValid()) {
             ...
             try {
                ...
                method.invoke(cache,
                      String.format("%s:%s", socket.getInetAddress().getHostName(), String.valueOf(socket.getPort()))
                            .toLowerCase(Locale.ROOT),
                      session);
                method.invoke(cache,
                      String.format("%s:%s", socket.getInetAddress().getHostAddress(), String.valueOf(socket.getPort()))
                            .toLowerCase(Locale.ROOT),
                      session);
