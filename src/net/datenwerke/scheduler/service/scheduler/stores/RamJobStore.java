...
public class RamJobStore extends JobStoreImpl {
   ...
   public Collection<AbstractJob> getMisfiredJobs() {
      ...
      for (AbstractJob job : jobSet) {
         ...
         if (null != scheduled && ((scheduled.before(now) && status.equals(JobExecutionStatus.INACTIVE))
               || job.getTrigger().isExecuteOnce()))
            jobs.add(job);
      }
      return jobs;
   }
   ...
   public Collection<AbstractJob> getExecutingJobs() {
      List<AbstractJob> jobs = new ArrayList<AbstractJob>();
      for (AbstractJob job : jobSet)
         if (JobExecutionStatus.EXECUTING.equals(job.getExecutionStatus())
               || JobExecutionStatus.EXECUTING_ACTIONS.equals(job.getExecutionStatus()))
            jobs.add(job);
      return jobs;
   }
