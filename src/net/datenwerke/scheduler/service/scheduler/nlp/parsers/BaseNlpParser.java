...
public class BaseNlpParser implements NlpTriggerParserHook {
   ...
   @Inject
   private Provider<DailyNthdayTrigger> dailyNthDayTriggerProvider;
   @Inject
   private Provider<DailyWorkdayTrigger> dailyWorkDayTriggerProvider;
   @Inject
   private Provider<WeeklyTrigger> weeklyTriggerProvider;
   @Inject
   private Provider<MonthlyNthDayMthMonthTrigger> monthlyTriggerProvider;
   ...
   private ExpressionAndSub<Integer> getMinuteExpression(StrTokenizer tokenizer) {
   ...
   private StrTokenizer appendTimeOfDay(DateTriggerConfig config, StrTokenizer tokenizer) {
      if (tokenizer.getTokenArray().length < 2)
   ...
   private StrTokenizer appendStarting(DateTriggerConfig config, StrTokenizer tokenizer) {
      if (tokenizer.getTokenArray().length <= 2)
         return tokenizer;
