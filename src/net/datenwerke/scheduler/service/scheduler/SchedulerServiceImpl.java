...
public class SchedulerServiceImpl implements SchedulerService {
   ...
   @Inject
   public SchedulerServiceImpl(@SchedulerDefaultJobStore JobStore defaultStore,
         @DwSchedulerConfig Provider<Configuration> schedulerConfig,

         EventBus eventBus, HookHandlerService hookHandler, Provider<Injector> injectorProvider,
         Provider<SchedulerDaemon> daemonProvider, Provider<SchedulerWatchdog> watchdogProvider,
         ConfigService configService, Provider<SecurityService> securityServiceProvider,
         Provider<AuthenticatorService> authenticatorServiceProvider, ApplicationPropertiesService propertiesService) {
   ...
   public void schedule(final AbstractJob job, final AbstractTrigger trigger) {
      ...
      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.beforeJobSchedule(job, trigger));

      jobStore.scheduleJob(job, trigger);

      eventBus.fireEvent(new JobScheduled(job));

      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.jobScheduled(job, trigger));
   }
   ...
   public void schedule(final AbstractJob job, final AbstractTrigger trigger, final AbstractJob previous) {
      ...
      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.jobRescheduled(job, trigger));
   }

   @Override
   public void unschedule(final AbstractJob job) {
      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.beforeJobUnschedule(job));

      jobStore.unschedule(job);

      eventBus.fireEvent(new JobUnscheduled(job));

      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.jobUnscheduled(job));
   }
   ...
   public void remove(final AbstractJob job) {
      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.beforeJobRemove(job));

      jobStore.remove(job);

      hookHandler.getHookers(SchedulerInfoHook.class).forEach(hooker -> hooker.jobRemoved(job));
   }
   ...
   private List<ReportScheduleJobListInformation> getBasicReportJobList(Report report,
         ReportServerJobFilter reportFilter) {
      List<ReportScheduleJobListInformation> jobsInfos = new ArrayList<>();
      JobStore jobStore = getJobStore();
      JobFilterConfiguration filterConf = new JobFilterConfiguration();
      filterConf.setInActive(true);
   ...
   @Override
   public void assertJobExecutorChangeAllowed(User previousExecutor, User newExecutor)
