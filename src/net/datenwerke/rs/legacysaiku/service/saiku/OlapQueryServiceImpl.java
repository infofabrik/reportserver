...
public class OlapQueryServiceImpl implements OlapQueryService {
   ...
   @Inject
   public OlapQueryServiceImpl(OlapUtilService olapUtilService) {
      this.olapUtilService = olapUtilService;
   }

   @Override
   public IQuery createNewOlapQuery(String queryName, SaikuReport report, String connectionName, Cube cube)
         throws SQLException {
      OlapConnection con = getOlapConnection(cube);
      SaikuCube saikuCube = new SaikuCube(connectionName, cube.getUniqueName(), cube.getName(), cube.getCaption(),
...
//					}
   ...
   @Override
   public List<SaikuMember> getResultMetadataMembers(IQuery query, boolean preferResult, String dimensionName,
      ...
      if (cs == null || !preferResult || members.size() == 0 || levels.size() == 1) {
         try {
            members = ObjectUtil.convertMembers(
                  olapUtilService.getAllMembers(query.getCube(), dimensionName, hierarchyName, levelName));
...
//				System.out.println("Using scenario:" + s + " : cell:" + position + " value" + value);
   ...
   public void moveDimension(IQuery query, String axisName, String dimensionName, int position) {
      try {
         if (log.isDebugEnabled()) {
            log.debug("move query: " + query.getName() + " dimension " + dimensionName + " to axis " + axisName
                  + "  position" + position);
         }
   ...
   public boolean removeAllChildren(IQuery query, String dimensionName) {
      ...
      try {
         ...
         throw new SaikuServiceException(
               "Cannot remove all children  for query (" + query.getName() + ") dimension (" + dimensionName + ")", e);
      }
   }

   @Override
   public boolean includeMember(IQuery query, String dimensionName, String uniqueMemberName, String selectionType,
         int memberposition) {
      ...
      try {
         ...
         throw new SaikuServiceException(
               "Cannot include member query (" + query.getName() + ") dimension (" + dimensionName + ") member ("
   ...
   @Override
   public boolean removeMember(IQuery query, String dimensionName, String uniqueMemberName, String selectionType)
   ...
   public boolean includeChildren(IQuery query, String dimensionName, String uniqueMemberName) {
      ...
      try {
         ...
         throw new SaikuServiceException("Cannot include children query (" + query.getName() + ") dimension ("
         ...
         for (List<Integer> cellPosition : cellPositions) {
            ...
            if (dimensions.size() == 0) {
               for (Member m : members) {
                  SaikuTupleDimension sd = new SaikuTupleDimension(m.getDimension().getName(),
