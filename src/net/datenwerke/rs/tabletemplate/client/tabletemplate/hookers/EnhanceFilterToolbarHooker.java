...
public class EnhanceFilterToolbarHooker implements FilterViewEnhanceToolbarHook {
   ...
   public interface TableTemplateTemplates extends XTemplates {
      @XTemplate("<div class=\"teamspace-nav-item x-combo-list-item\">"
            ...
            + "<div class=\"teamspace-nav-description\">{description:nullsafe}</div>" + "</div>")
   ...
   public void enhanceToolbarLeft(ToolBar toolbar) {
      ...
      DwTextButton editTemplateBtn = toolbarService.createSmallButtonLeft(messages.editTemplateBtn(),
            BaseIcon.TEMPLATE);
   ...
   private void loadTemplates() {
      ...
      tableTemplateDao.loadTemplates(report, filterView.getExecuteReportToken(),
            new RsAsyncCallback<List<TableReportTemplateDto>>() {
               @Override
   ...
   private ToolBar createToolbar() {
      ...
      DwTextButton removeTemplatesBtn = toolbarService.createSmallButtonLeft(messages.removeTemplateBtn(),
            BaseIcon.DELETE);
      removeTemplatesBtn.addSelectHandler(event -> {
         ...
         if (null != template) {
            ...
            tableTemplateDao.removeTemplates(report, filterView.getExecuteReportToken(), toRemove,
      ...
      DwTextButton downloadTemplatesBtn = toolbarService.createSmallButtonLeft(messages.downloadTemplateBtn(),
            BaseIcon.FLOPPY_O);
   ...
   protected void downloadTemplate() {
      TableReportTemplateDto template = grid.getSelectionModel().getSelectedItem();
      if (null != template) {
         String url = GWT.getModuleBaseURL() + TableTemplateUIModule.DOWNLOAD_SERVLET + "?"
               + TableTemplateUIModule.EXECUTE_REPORT_TOKEN + "=" + filterView.getExecuteReportToken() + "&"
               + TableTemplateUIModule.REPORT_ID + "=" + report.getId() + "&" + TableTemplateUIModule.TEMPLATE_ID + "="
               + template.getId() + "&" + TableTemplateUIModule.TEMPLATE_TEMPORARY_ID + "=" + template.getTemporaryId();
   ...
   private Grid<TableReportTemplateDto> createGrid() {
      ...
      ColumnConfig<TableReportTemplateDto, TableReportTemplateDto> descriptionConfig = new ColumnConfig<TableReportTemplateDto, TableReportTemplateDto>(
            new IdentityValueProvider<TableReportTemplateDto>(), 250, messages.templateName());
   ...
   protected FormPanel createEditForm(final SubmitCompleteCallback callback, final TableReportTemplateDto template) {
      ...
      if (null != template)
         templateTempIdField
               .setValue(null == template.getTemporaryId() ? "" : String.valueOf(template.getTemporaryId()));
      ...
      layoutContainer.add(new FieldLabel(nameField, BaseMessages.INSTANCE.propertyName()),
      ...
      layoutContainer.add(new FieldLabel(descriptionField, BaseMessages.INSTANCE.propertyDescription()),
            new VerticalLayoutData(1, -1));
      ...
      hookHandler.getHookers(TableTemplateClientProviderHook.class).forEach(provider -> {
      ...
      ListView<ModelClass, ModelClass> view = new ListView<ModelClass, ModelClass>(typeStore,
            new IdentityValueProvider<ModelClass>());
      view.setCell(new AbstractCell<ModelClass>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, ModelClass value, SafeHtmlBuilder sb) {
            sb.append(xtemplate.render(value));
         }
      });

      final ComboBox<ModelClass> templateTypeCombo = new ComboBox<>(
            new ComboBoxCell<>(typeStore, ModelClass::getName, view));
      ...
      if (null == template) {
         ...
         typeStore.getAll().stream().filter(model -> model.getType().equals(template.getTemplateType())).findAny()
      ...
      layoutContainer.add(new FieldLabel(contentTypeField, messages.templateContentTypeLabel()),
            new VerticalLayoutData(1, -1));
      ...
      layoutContainer.add(new FieldLabel(fileExtensionField, messages.fileExtensionLabel()),
            new VerticalLayoutData(1, -1));
