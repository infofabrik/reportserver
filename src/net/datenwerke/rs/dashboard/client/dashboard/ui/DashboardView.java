...
public class DashboardView extends DwFlowContainer {
   ...
   @Inject
   public DashboardView(HookHandlerService hookHandler, Provider<DadgetPanel> dadgetPanelProvider,
      ...
      for (DadgetDto dadget : dashboard.getDadgets()) {
         ...
         default:
            ((VerticalLayoutContainer) centerContainer
                  .getWidget(Math.max(0, Math.min(centerContainer.getWidgetCount() - 1, dadget.getCol())))).add(panel,
                        new VerticalLayoutData(1, height, new Margins(0, 0, 10, 0)));
         }
      }
   ...
   protected void updatePanelSize() {
      ...
      for (Widget w : topContainer) {
         ...
         ((DadgetPanel) w).setHeight((int) ((DadgetPanel) w).getDadget().getHeight()); // needed when swithching from
                                                                                       // scaled to unscaled
      }
   ...
   private void showTopBottomSinglePage() {
      ...
      /*
       * adapt sizes for this we take the number of dadgets on the longest path
       * (number of dadgets not height) and distribute everything equally
       */
      List<DadgetPanel> panels = new ArrayList<DadgetPanel>();
      for (Widget w : topContainer)
         panels.add((DadgetPanel) w);
