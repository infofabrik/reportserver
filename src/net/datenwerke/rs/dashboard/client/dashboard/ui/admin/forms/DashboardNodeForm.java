...
public class DashboardNodeForm extends SimpleFormView {
   ...
   class MyDashboardContainer implements DashboardContainer {
      ...
      public MyDashboardContainer(DwContentPanel wrapperPanel, DwContentPanel viewWrapper,
      ...
      @Override
      public void dadgetConfigured(final DashboardDto dashboard, final DadgetDto dadget, ConfigType type,
...
//			});
         ...
         if (type == ConfigType.CONFIG && (dadget instanceof ReportDadgetDto || dadget instanceof LibraryDadgetDto)) {
            ConfirmMessageBox cmb = new DwConfirmMessageBox(DashboardMessages.INSTANCE.storeDashboardPromptTitle(),
                  DashboardMessages.INSTANCE.storeToConfigure());
   ...
   @Inject
   public DashboardNodeForm(ToolbarService toolbarService, Provider<DashboardView> viewProvider,
         DashboardDao dashboardDao, DadgetCatalogFactory catalogFactory) {
      super();
   ...
   public void configureSimpleForm(final SimpleForm form) {
      form.setHeading(DashboardMessages.INSTANCE.editDashboardNode()
            + (getSelectedNode() == null ? "" : " (" + getSelectedNode().getId() + ")"));

      form.addField(String.class, DashboardNodeDtoPA.INSTANCE.name(), BaseMessages.INSTANCE.propertyName());

      form.addField(String.class, DashboardNodeDtoPA.INSTANCE.description(),
      ...
      final MyDashboardContainer container = new MyDashboardContainer(wrapperPanel, viewWrapper, viewProvider,
            (DashboardNodeDto) getSelectedNode());
      ...
      form.addField(CustomComponent.class, DashboardMessages.INSTANCE.dashboard(),
            new SFFCCustomComponentImpl(labeledWrapper));
      ...
      /* add dadget */
      DwTextButton addDadgetBtn = toolbarService.createSmallButtonLeft(DashboardMessages.INSTANCE.addDadget(),
            BaseIcon.DADGET);
