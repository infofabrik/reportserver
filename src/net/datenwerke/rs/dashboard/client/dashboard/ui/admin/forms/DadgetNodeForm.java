...
public class DadgetNodeForm extends SimpleFormView {
	...
	protected void onSubmit(final SimpleFormSubmissionCallback callback) {
		...
		if (null != adminFormKey && dadgetNode.getDadget() instanceof ReportDadgetDto) {
			...
			((ReportDadgetDto)dadget).setShowExecuteButton((boolean)adminForm.getValue(ReportDadgetProcessor.SHOW_EXECUTE_KEY));
			((ReportDadgetDto)dadget).setReport((ReportDto)adminForm.getValue(ReportDadgetProcessor.REPORT_PROPERTY_KEY));
		}
