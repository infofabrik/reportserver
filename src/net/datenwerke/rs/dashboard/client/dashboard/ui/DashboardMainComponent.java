...
public class DashboardMainComponent extends DwContentPanel implements DashboardContainer {
   ...
   protected void initToolbar() {
      ...
      hookHandler.getHookers(DashboardToolbarHook.class).forEach(hooker -> hooker.addLeft(toolbar, this));

      toolbar.add(new FillToolItem());

      hookHandler.getHookers(DashboardToolbarHook.class).forEach(hooker -> hooker.addRight(toolbar, this));
      ...
      final MenuItem editOrderDashboardItem = new DwMenuItem(DashboardMessages.INSTANCE.editDashboardOrder(),
            BaseIcon.ARROW_UP);
      ...
      /* import */
      MenuItem addReferencedDashboardItem = new DwMenuItem(DashboardMessages.INSTANCE.createReferenceDashboard(),
            BaseIcon.IMPORT);
      toolsMenu.add(addReferencedDashboardItem);
      addReferencedDashboardItem.addSelectionHandler(e -> createReferenceDashboard());

      resetReferencedDashboardItem = new DwMenuItem(DashboardMessages.INSTANCE.resetReferenceDashboard(),
            BaseIcon.ROTATE_LEFT);
      ...
      importDashboardPopup = new TreeSelectionPopup(dashboardTree, DashboardNodeDto.class) {
         ...
         protected void itemsSelected(List<AbstractNodeDto> selectedItems) {
            if (null != selectedItems && !selectedItems.isEmpty()) {
               dao.importReferencedDashboardForUser((DashboardNodeDto) selectedItems.get(0),
                     new RsAsyncCallback<DashboardDto>() {
                        @Override
   ...
   private native String getCatalogViewTemplate() /*-{
		...
		'</tpl>', '<div class="x-clear"></div>' ].join("");
   }-*/;
   ...
   protected void editDashboard() {
      ...
      form.addField(String.class, DashboardDtoPA.INSTANCE.description(), DashboardMessages.INSTANCE.descriptionLabel(),
      ...
      form.addField(Boolean.class, DashboardDtoPA.INSTANCE.singlePage(), DashboardMessages.INSTANCE.singlePageLabel(),
            new SFFCDashboardLayout());
      form.addField(Long.class, DashboardDtoPA.INSTANCE.reloadInterval(),
            DashboardMessages.INSTANCE.reloadIntervalLabel());
      form.addField(List.class, DashboardDtoPA.INSTANCE.layout(), DashboardMessages.INSTANCE.layoutLabel(),
            new SFFCDashboardLayout());
   ...
   protected void init(DashboardContainerDto result, DashboardDto dashboardToSelect) {
      ...
      if (null != dashboardToSelect)
         ...
         /*
          * We select the primary dashboard if set. If not, we select the first dashboard
          * in the list.
          */
   ...
   public void dadgetConfigured(DashboardDto dashboard, final DadgetDto dadget, ConfigType type,
         final EditSuccessCallback callback) {
   ...
   public void selected() {
      if (!loaded) {
         ...
         if (null != view) {
            ...
            Scheduler.get().scheduleDeferred(() -> view.doForceLayout());
         }
