...
public class DashboardUiStartup {
   ...
   @Inject
   public DashboardUiStartup(final HookHandlerService hookHandler, final WaitOnEventUIService waitOnEventService,
         final SecurityUIService securityService,

         @DashboardManagerAdminViewTree Provider<UITree> dashboardManagerTree, HistoryUiService historyService,
         EventBus eventBus, Provider<DashboardManagerPanel> dashboardAdminPanel, final DashboardDao dashboardDao,
      ...
      hookHandler.attachHooker(GenericTargetProviderHook.class,
            new GenericTargetProviderHook(new DashboardViewGenericTargetIdentifier()));
      hookHandler.attachHooker(GenericSecurityViewDomainHook.class, securityTargetDomain);

      hookHandler.attachHooker(GenericTargetProviderHook.class,
            new GenericTargetProviderHook(new DashboardAdminGenericTargetIdentifier()));
            ...
            new SynchronousCallbackOnEventTrigger() {
               ...
               public void execute(final WaitOnEventTicket ticket) {
                  if (securityService.hasRight(DashboardViewGenericTargetIdentifier.class, ReadDto.class)) {
                     ...
                     final ClientMainModuleProviderHook mainModule = new ClientMainModuleProviderHook(
                           mainModuleProvider);

                     /* attach hooker */
                     hookHandler.attachHooker(ClientMainModuleProviderHook.class, mainModule,
                           HookHandlerService.PRIORITY_HIGH + 5);

                     hookHandler.attachHooker(TsFavoriteMenuHook.class, markNodeAsFavoriteHooker,
               ...
               public void execute(final WaitOnEventTicket ticket) {
                  if (securityService.hasRight(DashboardAdminGenericTargetIdentifier.class, ReadDto.class)) {
                     hookHandler.attachHooker(AdminModuleProviderHook.class,
                           new AdminModuleProviderHook(adminModuleProvider), HookHandlerService.PRIORITY_HIGH + 50);
      ...
      historyService.addHistoryCallback(DashboardUiModule.DASHBOARD_HISTORY_TOKEN, new TreeDBHistoryCallback(
            dashboardManagerTree, eventBus, dashboardAdminPanel, AdministrationUIModule.ADMIN_PANEL_ID));
