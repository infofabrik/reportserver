...
public class ManagerHelperSearchBar implements ManagerHelperTreeToolbarEnhancerHook {
   ...
   @Inject
   public ManagerHelperSearchBar(SearchDao searcher, SearchUiService searchService, ToolbarService toolbarService) {
   ...
   protected void addSearchBar(ToolBar toolbar, final UITree tree, final TreeDbManagerContainer treeManagerContainer,
         final SelectionHandler<SearchResultEntryDto> selectionHandler) {
         ...
         public void load(SearchLoadConfig loadConfig, AsyncCallback<SearchResultListDto> callback) {
            ...
            if (null != query && query.length() > 1) {
               ...
               if (null == types || types.isEmpty())
                  ...
                  searcher.find(searchService.enhanceQuery(query), searchService.createFilterFor(types, false),
                        callback);
