...
public class AnalyticalFunctionWrapper {
   ...
   @Inject
   public AnalyticalFunctionWrapper(EntityClonerService entityClonerService, SimpleDataSupplier datasupplier,
   ...
   private List<QryCondition> _executeBTin(final int n, Order order, String groupedBy) {
      ...
      if (n >= 1) {
         try {
                  ...
                  public void consumeRow(Object[] row) {
                     ...
                     for (int i = 0; i < nrOfGroupRows; i++) {
                        if ((null == lastGroupColValues[i] && null != row[i])
                              || (null == row[i] && null != lastGroupColValues[i]) || (null != lastGroupColValues[i]
                                    && null != row[i] && !lastGroupColValues[i].equals(row[i]))) {
                           groupBreak = true;
                           break;
                        }
                     }
                     ...
                     if (groupBreak) {
                        /* add condition */
                        if (null != value) {
                           conditions.add(queryBuilder.getNewAndQueryCondition(
            ...
            /* loop over result */
            datasupplier.getColumnValuesPaged(datasourceContainerProvider, parameterSet, columnList, null, null, null,
                  false, consumer);
         } catch (ReportExecutorException e) {
            throw new ReportExecutorRuntimeException(e);
         }
   ...
   protected QryCondition buildEqualGroupCondition(List<Column> columnList, Object[] lastGroupColValues,
         int nrOfGroupRows) {
      if (nrOfGroupRows == 1)
         return queryBuilder.getNewEqualQueryCondition(query.getColumnByName(columnList.get(0).getName()),
               lastGroupColValues[0]);

      return queryBuilder.getNewAndQueryCondition(
            queryBuilder.getNewEqualQueryCondition(query.getColumnByName(columnList.get(nrOfGroupRows - 1).getName()),
                  lastGroupColValues[nrOfGroupRows - 1]),
