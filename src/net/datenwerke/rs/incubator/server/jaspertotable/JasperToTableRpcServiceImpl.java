 ...
 */
@Singleton
public class JasperToTableRpcServiceImpl extends SecuredRemoteServiceServlet implements JasperToTableRpcService {
   ...
   @Inject
   public JasperToTableRpcServiceImpl(DtoService dtoService, ReportService reportService) {
   ...
   public JasperToTableConfigDto getConfig(JasperReportDto reportDto) throws ServerCallFailedException {
   ...
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public void setConfig(JasperReportDto reportDto, JasperToTableConfigDto configDto) throws ServerCallFailedException {
      JasperReport report = (JasperReport) dtoService.loadPoso(reportDto);
      ...
      if (!isActive && null != property)
         ...
         if (null != configDto.getDatasourceContainer())
            ((JasperToTableConfig) property).setDatasourceContainer(
                  (DatasourceContainer) dtoService.loadAndMergePoso(configDto.getDatasourceContainer()));
      }
