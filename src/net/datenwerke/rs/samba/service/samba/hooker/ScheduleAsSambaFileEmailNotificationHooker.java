...
public class ScheduleAsSambaFileEmailNotificationHooker extends SchedulerExecutionHookAdapter {
   ...
   public static final String PROPERTY_SAMBA_NOTIFICATION_SUBJECT_SUCCESS = "scheduler.fileactionSamba.subject";
   public static final String PROPERTY_SAMBA_NOTIFICATION_TEXT_SUCCESS = "scheduler.fileactionSamba.text";

   private static final String PROPERTY_SAMBA_NOTIFICATION_DISABLED = "scheduler.fileactionSamba[@disabled]";
   private static final String PROPERTY_SAMBA_NOTIFICATION_HTML = "scheduler.fileactionSamba[@html]";
   ...
   @Inject
   public ScheduleAsSambaFileEmailNotificationHooker(@SchedulerModuleProperties Configuration config,
   ...
   public void actionExecutionEndedSuccessfully(AbstractJob abstractJob, AbstractAction abstractAction,
      ...
      if (!(abstractAction instanceof ScheduleAsSambaFileAction))
         return;

      ReportExecuteJob job = (ReportExecuteJob) abstractJob;

      try {
         sendmail((ScheduleAsSambaFileAction) abstractAction, job, null);
   ...
   private void sendmail(ScheduleAsSambaFileAction action, ReportExecuteJob job, Exception e)
         throws MessagingException {
      if (config.getBoolean(PROPERTY_SAMBA_NOTIFICATION_DISABLED, false))
         return;
      ...
      String subjectTemplate = config.getString(PROPERTY_SAMBA_NOTIFICATION_SUBJECT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionSambaMsgSubject"));
      String messageTemplate = config.getString(PROPERTY_SAMBA_NOTIFICATION_TEXT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionSambaMsgText"));

      MailTemplate mailTemplate = new MailTemplate();
      mailTemplate.setHtml(config.getBoolean(PROPERTY_SAMBA_NOTIFICATION_HTML, false));
