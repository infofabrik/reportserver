...
@Entity
@Table(name = "SCHED_ACTION_AS_SAMBA_FILE")
@Inheritance(strategy = InheritanceType.JOINED)
public class ScheduleAsSambaFileAction extends AbstractAction {
   ...
   public void execute(AbstractJob job) throws ActionExecutionException {
      ...
      sendViaSambaDatasink(rJob, filename);

      if (null == name || name.trim().isEmpty())
         throw new ActionExecutionException("name is empty");

      if (null == sambaDatasink)
         throw new ActionExecutionException("samba datasink is empty");
   ...
   private void sendViaSambaDatasink(final ReportExecuteJob rJob, final String filename)
         throws ActionExecutionException {
      datasinkService.exportIntoDatasink(rJob, compressed, sambaDatasink, new DatasinkFilenameFolderConfig() {
   ...
   public SambaDatasink getSambaDatasink() {
      return sambaDatasink;
   }

   public void setSambaDatasink(SambaDatasink sambaDatasink) {
      this.sambaDatasink = sambaDatasink;
   }
}
