...
public class MemoryInfoPanel extends DwContentPanel {
	...
	private void createForm() {
		...
		form.beginFloatRow();
		numberOfUnitsKey = form.addField(Integer.class, SystemConsoleMessages.INSTANCE.numberUnits(), new SFFCAllowBlank() {
			@Override
		...
		unitIntervalKey = form.addField(Integer.class, SystemConsoleMessages.INSTANCE.unitInterval(), new SFFCAllowBlank() {
			@Override
			public boolean allowBlank() {
				return false;
			}
	...
	protected void updateChart(final MemoryInfoDto memoryInfo) {
		...
		if (numberOfUnitsToDelete > 0) {
			IntStream
				.range(0, numberOfUnitsToDelete)
				.forEach(i -> store.remove(i));
		}
