...
public class ReportDocumentationServlet extends SecuredHttpServlet {
	...
	private void getVariantTestDocumentation(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		List<DatasourceDefinition> datasources = 
			streamOfNullable(ds)
			.map(dsName -> datasourceServiceProvider.get().getDatasourceByName(dsName))
			.collect(toList());
		
		/* get format */
		String format = null != request.getParameter(PROPERTY_FORMAT) ? request.getParameter(PROPERTY_FORMAT) : "HTML";
		try {
			CompiledReport compiledReport = documentationServiceProvider.get().executeDocumentationVariantTest(report, datasources, format);
	...
	protected void getDeployAnalyzeDocumentation(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			CompiledReport compiledReport = documentationServiceProvider.get().executeDocumentationDeployAnalyze(leftReport, rightReport, format, ignoreCase);
