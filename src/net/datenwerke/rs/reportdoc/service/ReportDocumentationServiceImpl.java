...
public class ReportDocumentationServiceImpl implements ReportDocumentationService {
   ...
   @Inject
   public ReportDocumentationServiceImpl(Provider<ReportDocGroovyHelper> groovyHelperProvider,
         Provider<ReportDocPdfRenderer> pdfRendererProvider, Provider<ServerInfoContainer> serverInfoProvider,
   ...
   @Override
   public CompiledReport executeDocumentationDeployAnalyze(Report leftReport, Report rightReport, String format,
   ...
   @Override
   public CompiledReport executeDocumentationVariantTest(Report report, List<DatasourceDefinition> datasources,
      ...
      List<Long> datasourceIds = datasources.stream().map(DatasourceDefinition::getId).collect(toList());
   ...
   protected CompiledScript compile(String location) throws IOException, ScriptException {
      String report = IOUtils
            .toString(getClass().getResourceAsStream(ReportDocumentationService.RESOURCES_DIR + location));
