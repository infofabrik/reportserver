...
public class VFSLocation implements Serializable {
   ...
   public VFSLocation(String path, VirtualFileSystemManagerHook vfsManager) {
   ...
   public Collection<VFSLocation> resolveWildcards(VirtualFileSystemDeamon vfs) {
      ...
      for (VFSObjectInfo childInfo : parentInfo.getChildInfos()) {
         if ((null == childInfo.getName() && "".matches(regex))
               || (null != childInfo.getName() && childInfo.getName().matches(regex))) {
   ...
   protected void setVirtualContentProvider(VirtualContentProviderHook virtualContentProvider) {
   ...
   public boolean mkdir() throws VFSException {
      ...
      VFSLocation created = getFilesystemManager().createFolder(this.getParentLocation(),
            getPathHelper().getLastPathway());
