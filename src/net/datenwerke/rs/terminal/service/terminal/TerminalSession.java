...
public class TerminalSession implements Serializable {
   ...
   public AutocompleteResult autocomplete(String command, int cursorPosition)
         throws MaxAutocompleteResultsExceededException {
      return autocomplete(command, cursorPosition, false);
   }

   public AutocompleteResult autocomplete(String command, int cursorPosition, boolean forceResult)
         throws MaxAutocompleteResultsExceededException {
      if (null != hijacker)
      ...
      deamonMap.values().forEach(deamon -> deamon.autocomplete(autoHelper));
   ...
   public CommandResult execute(String command, ExecuteCommandConfig config) throws TerminalException {
      ...
      for (TerminalCommandHook commandHooker : hookHandlerService.getHookers(TerminalCommandHook.class)) {
         if (commandHooker.consumes(parser, this)) {
            ...
            if (config.allowInteractive() && commandHooker instanceof InteractiveCommandHook
                  && ((InteractiveCommandHook) commandHooker).isKeepInteractiveSession()) {
            ...
            if (config.allowHijackers()) {
               for (TerminalSessionHijackHook hijackHooker : hookHandlerService
                     .getHookers(TerminalSessionHijackHook.class)) {
