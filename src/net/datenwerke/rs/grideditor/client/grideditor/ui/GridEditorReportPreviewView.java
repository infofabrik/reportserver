...
public class GridEditorReportPreviewView extends AbstractReportPreviewView implements PageablePreviewView, CloseableAware {
	...
	private final NumberField<Integer> pageField = new DwNumberField<Integer>(new NumberPropertyEditor.IntegerPropertyEditor());
	...
	protected ToolBar createToolbar() {
		...
		if(editorConfig.isCanRemoveRecords()) {
			...
			deleteButton.addSelectHandler( event -> 
				grid.getSelectionModel().getSelectedItems().forEach( this::markRemoveRecord )
			);
		...
		saveButton.addSelectHandler( event -> {
			...
			modified = modified.stream()
					.filter( rec -> ! newRecords.contains(rec) )
