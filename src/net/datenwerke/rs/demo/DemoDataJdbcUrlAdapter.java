...
public class DemoDataJdbcUrlAdapter extends JdbcUrlAdapterHookAdapter {
   ...
   @Inject
   public DemoDataJdbcUrlAdapter(DbPoolService dbPoolService, DBHelperService dbHelperService,
         @TempDirLocation Provider<String> tempDir, ApplicationPropertiesService propertiesService) {
...
//			ml.load();
   ...
   protected Future<Connection> getDemoConnection() throws SQLException {
      return dbPoolService.getConnection(
            new DemoDbConnectionPool(getDemoJdbcConnectionUrl(), null, getDemoJdbcConnectionUrl()),
            new StandardConnectionConfig());
   }
   ...
   private String getDemoJdbcConnectionUrl() {
      ...
      if (null != url) {
         url = url.replace("\\", "/"); // Windows: RS-2780
                                       // https://stackoverflow.com/questions/5784895/java-properties-backslash
      ...
      try {
         ...
         String[] files = new String[] { "rs_demo_data_db.mv.db", "rs_demo_data_db.trace.db", "rs_demo_data_db.lock.db",
               "rs_demo_data_db.h2.db" };
         for (String f : files) {
         ...
         url = "jdbc:h2:file:" + tmpFile.getAbsoluteFile().toString().replace("\\", "/"); // Windows: RS-2780
                                                                                          // https://stackoverflow.com/questions/5784895/java-properties-backslash
         logger.info("Demodata URL: " + url);
         return url;
      } catch (Exception e) {
