...
public class FileSelectionParameterAdjustVariantHooker implements VariantCreatorHook {
   ...
   public void temporaryVariantCreated(Report referenceReport, Report adjustedReport, Report variant) {
      List<FileSelectionParameterDefinition> definitions = referenceReport
            .getParameterDefinitionsOfType(FileSelectionParameterDefinition.class);
      if (null == definitions || definitions.isEmpty())
         return;

      /* get report from which variant is to be created */
      Report originalReport = reportService.getReportById(
            null == adjustedReport.getId() ? adjustedReport.getOldTransientId() : adjustedReport.getId());

      /* ensure that content of uploaded files is correctly set */
      for (FileSelectionParameterDefinition def : definitions) {
         FileSelectionParameterInstance instance = (FileSelectionParameterInstance) variant
               .getParameterInstanceFor(def);
         if (instance.isStillDefault())
            continue;

         ParameterInstance<?> referenceInstance = originalReport.getParamInstanceByDefinitionId(
               instance.getDefinition().getId() == null ? instance.getDefinition().getOldTransientId()
         ...
         /*
          * go over files and fix content, i.e., copy content from ref instance if
          * necessary
          */
         ...
         for (Iterator<SelectedParameterFile> iterator = selectedFiles.iterator(); iterator.hasNext();) {
            ...
            if (null != file.getUploadedFile() && null == file.getUploadedFile().getContent()) {
               ...
               UploadedParameterFile refFile = refInstance.getUploadedFileById(
                     uploadedFile.getId() == null ? uploadedFile.getOldTransientId() : uploadedFile.getId());
               if (null != refFile)
