...
/**
 * 
 * This should be a dto post processor.. no idea why this is implemented in such
 * an annoying way.
 */
@Deprecated
public class FileSelectionParameterDtoHelper {
   ...
   @Inject
   public FileSelectionParameterDtoHelper(FileUploadService uploadService, DtoService dtoService,
         EntityClonerService entityCloner, Provider<Injector> injectorProvider) {
   ...
   public void adaptPoso(ReportDto reportDto, Report report) {
      List<FileSelectionParameterDefinition> definitions = report
      ...
      for (FileSelectionParameterDefinition def : definitions) {
         ...
         FileSelectionParameterDefinitionDto defDto = (FileSelectionParameterDefinitionDto) reportDec
               .getParameterDefinitionByKey(key);
         if (null == defDto)
            continue;

         FileSelectionParameterInstanceDtoDec instanceDto = (FileSelectionParameterInstanceDtoDec) reportDec
               .getParameterInstanceFor(defDto);
         if (null == instanceDto)
            continue;
   ...
   public void fixInstanceFromDto(FileSelectionParameterInstance instance,
         FileSelectionParameterInstanceDtoDec instanceDto) {
         ...
         if (null != originalDto) {
            ...
            if (fileSize > 0 && uploadedFile.getFileBytes().length > fileSize)
               throw new IllegalArgumentException(
                     "File size too big. Allowed is at most " + definition.getFileSizeString());
