...
public class DatasourceParameterRPCServiceImpl extends SecuredRemoteServiceServlet
   ...
   @Inject
   public DatasourceParameterRPCServiceImpl(ReportParameterService parameterService,
         DatasourceParameterService datasourceParameterService, DtoService dtoService, SecurityService securityService,
         HookHandlerService hookHandler, ParameterSetFactory parameterSetFactory) {
   ...
   /**
    * if dependsOnParameterDTOs is null we'll use the depend's on stored in the
    * databse.
    */
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public ListLoadResult<DatasourceParameterDataDto> loadDatasourceParameterData(
         DatasourceParameterDefinitionDto parameterDto, Collection<ParameterInstanceDto> dependsOnParameterDTOs,
      ...
      if (null == reportDto) {
         /*
          * Sometimes reportDto is not available e.g. when it is not selected in the
          * tree, as there is no tree available as in the dashboards.
          */
      ...
      if (!validateParameter(unmanagedParameter, parameterDto))
         throw new RuntimeException(
               new ReportExecutorException(ReportManagerMessages.INSTANCE.exceptionInvalidParameter()));
      ...
      try {
         ...
         if (null == dependsOnParameterDTOs || dependsOnParameterDTOs.isEmpty()) {
            parameterData = datasourceParameterService
                  .getParameterData((DatasourceParameterDefinition) unmanagedParameter, report);
         } else {
            ...
            for (ParameterInstanceDto dependsOnParameterDTO : dependsOnParameterDTOs) {
               ...
               ParameterInstance dependsOnParameter = (ParameterInstance) dtoService
                     .createUnmanagedPoso(dependsOnParameterDTO);
               ...
               if (null != dependsOnParameterDTO.getDtoId()) {
                  ...
                  dependsOnParameter
                        .setDefinition(parameterService.getUnmanagedParameter(storedInstance.getDefinition()));
               }
            ...
            parameterData = datasourceParameterService
                  .getParameterData((DatasourceParameterDefinition) unmanagedParameter, parameterSet);
         }
   ...
   private boolean validateParameter(ParameterDefinition referenceDefinition,
         DatasourceParameterDefinitionDto recievedDefinition) {
      return true;
   }
