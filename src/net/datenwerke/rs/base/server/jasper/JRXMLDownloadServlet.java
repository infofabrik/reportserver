...
public class JRXMLDownloadServlet extends SecuredHttpServlet {
   ...
   @Inject
   public JRXMLDownloadServlet(Provider<ReportService> reportManagerProvider,
         Provider<AuthenticatorService> authenticatorServiceProvider, Provider<SecurityService> securityServiceProvider,
         Provider<ZipUtilsService> zipUtilsProvider, Provider<HttpUtils> httpUtilsProvider) {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if (rmn instanceof JasperReport) {
         ...
         response.setHeader(HttpUtils.CONTENT_DISPOSITION,
               httpUtilsProvider.get().makeContentDispositionHeader(true, report.getName() + "-jrxmlfiles.zip"));
   ...
   private void addFileToContent(Map<String, Object> contentMap, JasperReportJRXMLFile file)
         throws UnsupportedEncodingException {
      if (null != file) {
         byte[] content = file.getContent().getBytes("UTF-8");
         contentMap.put(file.getName() + (null != file.getName() && file.getName().endsWith(".jrxml") ? "" : ".jrxml"), //$NON-NLS-1$ //$NON-NLS-2$
