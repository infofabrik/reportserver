...
public class DatasourceParameterDao extends Dao {
   ...
   @Inject
   public DatasourceParameterDao(DatasourceParameterRPCServiceAsync rpcService, DatasourceUIService dsService,
   ...
   public void loadDatasourceParameterData(DatasourceParameterDefinitionDto parameterDefinition,
         Collection<ParameterInstanceDto> dependsOnParameterDTOs, boolean mergeDefinition, ReportDto reportDto,
      ...
      if ((null == parameterDefinition.getDatasourceContainer()
            || null == parameterDefinition.getDatasourceContainer().getDatasource())
            && (!dsParamService.isAllowPostProcessing() || null == parameterDefinition.getPostProcess()
                  || "".equals(parameterDefinition.getPostProcess().trim()))) {
      ...
      if (null != parameterDefinition.getDatasourceContainer()
            && null != parameterDefinition.getDatasourceContainer().getDatasource()) {
         DatasourceDefinitionConfigConfigurator configurator = dsService
               .getConfigurator(parameterDefinition.getDatasourceContainer().getDatasource().getClass());
      ...
      DaoAsyncCallback<ListLoadResult<DatasourceParameterDataDto>> transformedCallback = transformAndKeepCallback(
            callback);
      transformedCallback.setIgnoreAllExceptions(true);

      rpcService.loadDatasourceParameterData(parameterDefinition, unproxiedParameters, mergeDefinition, reportDto,
            transformedCallback);
   }
