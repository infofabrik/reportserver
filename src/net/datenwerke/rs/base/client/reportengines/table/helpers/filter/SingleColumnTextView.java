...
public class SingleColumnTextView extends TextView<StringBaseModel> {
   ...
   public SingleColumnTextView(final ListStore<StringBaseModel> store, ColumnDto column,
         SelectionPanel<StringBaseModel> selectionPanel, TabPanel tabPanel) {
      ...
      store.addStoreHandlers(new GenericStoreHandler<StringBaseModel>() {
         ...
         protected void handleDataChangeEvent() {
            ...
            for (StringBaseModel v : store.getAll())
               buf.append(filterService.getStringValue(v.getValue(), SingleColumnTextView.this.column.getType()))
                     .append("\r\n"); //$NON-NLS-1$
