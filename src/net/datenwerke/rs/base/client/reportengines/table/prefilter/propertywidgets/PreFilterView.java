...
public class PreFilterView extends ReportExecutorMainPanelView {
      ...
      store.addStoreAddHandler(new StoreAddHandler<RsDto>() {
         ...
         public void onAdd(StoreAddEvent<RsDto> event) {
            for (RsDto item : event.getItems()) {
               ...
               if (null == parentBlockModel && store.getRootItems().size() > 0
                     && !store.getRootItems().get(0).equals(item))
   ...
   private void createTableGrid() {
      /* create columns */
      ColumnConfig<RsDto, RsDto> name = new ColumnConfig<RsDto, RsDto>(new IdentityValueProvider<RsDto>("__name"), 200,
            BaseMessages.INSTANCE.name());
      name.setCell(new AbstractCell<RsDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, RsDto model, SafeHtmlBuilder sb) {
      ...
      TreeGridDropTarget<RsDto> target = new TreeGridDropTarget<RsDto>(treegrid) {
         ...
         protected void onDragMove(DndDragMoveEvent event) {
            try {
               ...
               final TreeNode<RsDto> targetNode = treegrid
         ...
         protected void onDragEnter(DndDragEnterEvent event) {
            try {
               ...
               final TreeNode<RsDto> targetNode = treegrid
                     .findNode(event.getDragEnterEvent().getNativeEvent().getEventTarget().<Element>cast());
   ...
   private void createToolbar() {
      ...
      /* add filter */
      DwTextButton addFilterButton = toolbarService.createSmallButtonLeft(messages.addFilterLabel(),
            BaseIcon.FILTER_ADD);
   ...
   protected void toggleAndOr() {
      ...
      report.getPreFilter()
            .setRootBlockType(root.getBlockType() == BlockTypeDto.OR ? BlockTypeDto.AND : BlockTypeDto.OR);
   ...
   protected void addFilter(final PreFilterConfiguratorHook filterConfig) {
      ...
      filterConfig.instantiateFilter(report, getExecuteReportToken(), new InstantiatePreFilterCallback() {
         ...
         public void filterInstantiated(FilterSpecDto filter) {
            ...
            filterConfig.filterInstantiated(report, (FilterSpecDto) filter, getExecuteReportToken(),
                  new EditPreFilterCallback() {
                     @Override
   ...
   protected void displayEditFilterDialog() {
      ...
      for (PreFilterConfiguratorHook filterConfig : preFilterConfigs) {
         if (filterConfig.consumes((FilterSpecDto) item)) {
            mainPanel.mask();
            filterConfig.displayFilter(report, (FilterSpecDto) item, getExecuteReportToken(),
                  new EditPreFilterCallback() {
                     @Override
                     public void editDone() {
                        mainPanel.unmask();
