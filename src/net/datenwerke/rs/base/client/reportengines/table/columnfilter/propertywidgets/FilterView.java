...
public class FilterView extends ReportExecutorMainPanelView implements SelectionAwareView {
   ...
   @Inject
   public FilterView(ClipboardUiService clipboardService, TableReportUtilityDao tableReportUtilityDao,
         ToolbarService toolbarService, GridHelperService gridHelperService, ContextHelpUiService contextHelpService,
         HookHandlerService hookHandler, StatementManagerDao statementManager, SqlTypes sqlTypes,
   ...
   private void createGrid() {
      ...
      convertToPreFilter.addSelectionHandler(new SelectionHandler<Item>() {
         @Override
         public void onSelection(SelectionEvent<Item> event) {
            final ConfirmMessageBox cmb = new DwConfirmMessageBox(FilterMessages.INSTANCE.convertToPrefilterTitle(),
                  FilterMessages.INSTANCE.convertToPrefilterText());
            cmb.addDialogHideHandler(new DialogHideHandler() {
               ...
               public void onDialogHide(DialogHideEvent event) {
                  ...
                  if (convert) {
                     ...
                     report.fireObjectChangedEvent();
                     Info.display(PreFilterMessages.INSTANCE.infoTitleColumnToPrefilter(),
                           PreFilterMessages.INSTANCE.infoTextColumnToPrefilter());
                  }
               }
            });
   ...
   private void createToolbar() {
      ...
      DwTextButton selectColumnsButton = toolbarService.createSmallButtonLeft(FilterMessages.INSTANCE.selectColumns(),
            BaseIcon.TABLE_ADD);
...
//		contentPanel.setWidget(grid);
   ...
   protected void importColumn(ColumnDto col) {
      ...
      if (null == availableColumnsInReport) {
         ...
         if (found)
            ...
            new DwAlertMessageBox(FilterMessages.INSTANCE.columnCannotBeImportedTitle(),
                  FilterMessages.INSTANCE.columnNotAllowedInReport(name)).show();
   ...
   private CCContainer<ColumnDto, AggregateFunctionDto> addAggregateColumn(
         final List<ColumnConfig<ColumnDto, ?>> columns) {
      final CCContainer<ColumnDto, AggregateFunctionDto> cccAgg = gridHelperService.createComboBoxColumnConfig(
            AggregateFunctionDto.values(), ColumnDtoPA.INSTANCE.aggregateFunction(), true, SortDir.ASC, 120);
   ...
   private void configAggregateColumnEditing(final List<ColumnConfig<ColumnDto, ?>> columns,
         CCContainer<ColumnDto, AggregateFunctionDto> cccAgg, GridEditing<ColumnDto> editing) {
      final ColumnConfig<ColumnDto, AggregateFunctionDto> aggreagateColumn = cccAgg.getConfig();
      ...
      editing.addCompleteEditHandler(new CompleteEditHandler<ColumnDto>() {
         ...
         public void onCompleteEdit(CompleteEditEvent<ColumnDto> event) {
            ...
            if (null != col && sqlTypes.isLob(col.getType()) && aggreagateColumn == gridColumn
                  && col.getAggregateFunction() != null) {
   ...
   private CCContainer<ColumnDto, OrderDto> addOrderColumn(List<ColumnConfig<ColumnDto, ?>> columns) {
      CCContainer<ColumnDto, OrderDto> ccOrderContainer = gridHelperService
            .createComboBoxColumnConfig(OrderDto.values(), ColumnDtoPA.INSTANCE.order(), true, null, 90);
   ...
   private void configOrderColumnEditing(CCContainer<ColumnDto, OrderDto> ccOrderContainer,
   ...
   private CCContainer<ColumnDto, Boolean> addVisibilityColumn(List<ColumnConfig<ColumnDto, ?>> columns) {
      CCContainer<ColumnDto, Boolean> cccBoolean = gridHelperService.createBooleanComboBoxColumnConfig(
            ColumnDtoPA.INSTANCE.hidden(), false, false, 90, FilterMessages.INSTANCE.hidden(),
            FilterMessages.INSTANCE.visible());
      ...
      visibleColumn.setCell(new SimpleSafeHtmlCell<Boolean>(new AbstractSafeHtmlRenderer<Boolean>() {
         ...
         public SafeHtml render(Boolean object) {
            return SafeHtmlUtils
                  .fromString(object ? FilterMessages.INSTANCE.hidden() : FilterMessages.INSTANCE.visible());
         }
      }));
   ...
   private void configVisibilityColumnEditing(CCContainer<ColumnDto, Boolean> cccBoolean,
         GridEditing<ColumnDto> editing) {
   ...
   private void addRecommendedWidthColumn(List<ColumnConfig<ColumnDto, ?>> columns) {
      ColumnConfig<ColumnDto, String> ccDefaultWidth = new ColumnConfig<ColumnDto, String>(
            ColumnDtoPA.INSTANCE.defaultPreviewWidth(), 170, FilterMessages.INSTANCE.defaultWidth());
   ...
   private ColumnConfig<ColumnDto, Integer> addWidthColumn(List<ColumnConfig<ColumnDto, ?>> columns) {
      ColumnConfig<ColumnDto, Integer> ccWidth = new ColumnConfig<ColumnDto, Integer>(
            ColumnDtoPA.INSTANCE.previewWidth(), 100, FilterMessages.INSTANCE.width());
         ...
         public Integer convertFieldValue(String object) {
            if (null == object)
               ...
               if (intValue > maxColumnWidth) {
                  new DwAlertMessageBox(FilterMessages.INSTANCE.failed(),
                        FilterMessages.INSTANCE.previewWidthWarningMsg() + maxColumnWidth).show();
                  return null;
               }
               return intValue;
            }
