...
public class DistinctSelectorPanel<M> {
   ...
   public DistinctSelectorPanel(TableReportDto report, ColumnDto column, SelectionPanel<M> selectionPanel,
         FilterType type, String executeToken) {
      this.type = type;
      this.report = report;
      this.column = column;
      this.selectionPanel = selectionPanel;
      this.executeToken = executeToken;
      this.countFilterValues = "auto".equals(report
            .getEffectiveReportProperty(AvailableReportProperties.PROPERTY_DL_FILTER_COUNT_DEFAULT.getValue(), "auto"));
   ...
   private void createGrid() {
      RpcProxy<SelectorPanelLoadConfig<M>, PagingLoadResult<StringBaseModel>> proxy = new RpcProxy<SelectorPanelLoadConfig<M>, PagingLoadResult<StringBaseModel>>() {
         ...
         public void load(SelectorPanelLoadConfig<M> loadConfig,
            ...
            loadConfig.setCaseSensitive(null != column.getFilter() && null != column.getFilter().isCaseSensitive()
                  ? column.getFilter().isCaseSensitive()
                  : true);

            request = tableReportUtilityDao.getDistinctValuesPaged(loadConfig, report, column, type,
                  Boolean.TRUE.equals(forceConsistent.getValue()), countFilterValues, executeToken, callback);
         }
      };
      ...
      ListStore<StringBaseModel> store = new ListStore<StringBaseModel>(
            new BasicObjectModelKeyProvider<StringBaseModel>());
      loader.addLoadHandler(
            new LoadResultListStoreBinding<SelectorPanelLoadConfig<M>, StringBaseModel, PagingLoadResult<StringBaseModel>>(
                  store));
...
//		((TextBox)pagingToolBar.getWidget(1)).setHeight("27px");
      ...
      ColumnConfig<StringBaseModel, String> valueConfig = new ColumnConfig<StringBaseModel, String>(sbmPa.value(), 250,
            BaseMessages.INSTANCE.value());
   ...
   private void createToolbar() {
      ...
      final TriggerField<StringBaseModel> textFilter = new TriggerField<StringBaseModel>(
            new PropertyEditor<StringBaseModel>() {
...
//		textFilter.setTriggerStyle("x-form-clear-trigger");
      ...
      textFilter.addValidator(new Validator<StringBaseModel>() {
         ...
         public List<EditorError> validate(Editor<StringBaseModel> editor, StringBaseModel valueModel) {
            ...
            /* set filter */
            plc.setFilter(value.replace("*", "%").replace("?", "_") + "%"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
                                                                            // //$NON-NLS-6$
      ...
      if (SqlTypes.isDateLikeType(column.getType()))
         toolbar.add(SeparatorTextLabel.createSmallText(FilterMessages.INSTANCE.noSearchPossible()),
               new BoxLayoutData(new Margins(0, 0, 10, 0)));
      else
