...
public class ColumnQuery extends CompositeQuery {
   ...
   public ColumnQuery(Query nestedQuery, QueryBuilder queryBuilder, ColumnNamingService baseNameService,
   ...
   public void appendToBuffer(StringBuffer buf) {
      if ((null == queryBuilder.getColumns() || queryBuilder.getColumns().size() == 0)
            && (null == queryBuilder.getAdditionalColumns() || queryBuilder.getAdditionalColumns().size() == 0)) {
      ...
      if (queryBuilder.getColumns().size() == 0) {
         ...
         for (Column col : queryBuilder.getColumns()) {
            ...
            if (!(col instanceof ColumnReference)) {
               ...
               boolean found = false;
               for (ColumnReferenceSqlProvider sqlProvider : queryBuilder.getHookHandler()
                     .getHookers(ColumnReferenceSqlProvider.class)) {
      ...
      /*
       * if we have additional columns we have an extra level in the query. In this
       * case we have to specifically select the columns that are used in prefilters
       * for the where clause to work.
       */
      if (queryBuilder.getColumns().size() > 0 && !queryBuilder.getAdditionalColumns().isEmpty()
            && !queryBuilder.getInnerConditions().isEmpty()) {
