...
public class ManagedQuery {
   ...
   @Inject
   public ManagedQuery(HookHandlerService hookHandler, FilterExecutorHooker filterExecutor, EventBus eventBus,
         @Assisted String query, @Assisted DatabaseHelper dbHelper, @Assisted @Nullable TableDBDataSource datasource) {
...
//		System.out.println(prelimStmt);
      ...
      try {
         ...
         ReportExecutorException ree = new ReportExecutorException(
               DatasourcesMessages.INSTANCE.exceptionCouldNotPrepareStmt(e.getLocalizedMessage()), e);
         throw ree;
      }
   ...
   public String preparePrelimStatement(Map<String, ParameterValue> pMap, Connection connection)
         throws SQLException, ReportExecutorException {
      /* get prefixer object */
      ...
      if (null != columns) {
         for (Column column : columns) {
            ...
            cf.setColumn(column);
            QryCondition innerCondition = filterExecutor.getQueryCondition(cf, pMap, prefixer, queryBuilder, this,
                  connection);
   ...
   private QryCondition addPreFilter(QueryBuilder queryBuilder, FilterBlock filterBlock,
         Map<String, ParameterValue> pMap, QueryReplacementHelper prefixer, Connection connection) {
      if (null == filterBlock)
         return null;
         ...
         for (FilterSpec filter : filterBlock.getFilters()) {
            ...
            for (DbFilterExecutorHook filterExecutor : hookHandler.getHookers(DbFilterExecutorHook.class)) {
               if (filterExecutor.consumes(filter)) {
                  innerCondition = filterExecutor.getQueryCondition(filter, pMap, prefixer, queryBuilder, this,
                        connection);
                  break;
               }
            }
   ...
   public void setAdditionalColumnSpecs(List<AdditionalColumnSpec> additionalColumns) {
