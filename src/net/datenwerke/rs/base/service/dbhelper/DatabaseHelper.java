 ...
 *
 */
@GenerateDto(dtoPackage = "net.datenwerke.rs.base.client.dbhelper.dto", additionalFields = {
...
abstract public class DatabaseHelper {
   ...
   public Query buildQuery(final QueryBuilder builder) {
      ...
      ColumnNamingService uniqueNameService = new ColumnNamingService() {
         ...
         public String getColumnName(Column column) {
            if (column instanceof AdditionalColumnSpec)
               return getIdentifierQuoteChar()
                     + builder.getUniqueAddColumnSpecIdentifier(((AdditionalColumnSpec) column))
      ...
      ColumnNamingService baseNameService = new ColumnNamingService() {
         ...
         public String getColumnName(Column column) {
            if (column instanceof ColumnReference)
               return getIdentifierQuoteChar()
                     + builder.getUniqueAddColumnSpecIdentifier(((ColumnReference) column).getReference())
                     + getIdentifierQuoteChar();
      ...
      case SELECT:
         ...
         /*
          * select columns: As we cannot be sure that the query is a simple SELECT A FROM
          * B we have to wrap it in any case. If we do not perform column selection we
          * use a dummy wrapper
          */
   ...
   public QueryBuilder createNestedQuery(String innerQuery, Map<String, ParameterValue> parameteReplacementMap,
         QueryReplacementHelper queryReplacementHelper, List<String> plainColumnNames) {
   ...
   public AggregateHavingQuery getNewAggregateHavingQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService columnNamingService) {
      return new AggregateHavingQuery(nestedQuery, queryBuilder, columnNamingService);
   }

   public ColumnFilterQuery getNewColumnFilterQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService uniqueNameService) {
      return new ColumnFilterQuery(nestedQuery, queryBuilder, uniqueNameService);
   }

   public ColumnQuery getNewColumnQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService baseNameService, ColumnNamingService uniqueNameService) {
   ...
   public FilterWhereQuery getNewFilterWhereQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService columnNamingService) {
      return new FilterWhereQuery(nestedQuery, queryBuilder, columnNamingService);
   }

   public InnerFilterWhereQuery getNewInnerFilterWhereQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService uniqueNameService) {
   ...
   public OrderLimitQuery getNewOrderLimitQuery(Query nestedQuery, QueryBuilder queryBuilder,
         ColumnNamingService columnNamingService) {
