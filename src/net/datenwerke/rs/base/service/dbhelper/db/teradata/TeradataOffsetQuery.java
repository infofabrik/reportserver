...
public class TeradataOffsetQuery extends OffsetQuery {
   ...
   /*
    * SELECT "xx__rs_col_0", "xx__rs_col_1" FROM (SELECT teradataOffQry1.*,
    * (ROW_NUMBER() OVER(ORDER BY 1)) rowNum FROM (SELECT "xx__rs_col_0",
    * "xx__rs_col_1" FROM ( SELECT AccessRight AS "xx__rs_col_0", CreateTimeStamp
    * AS "xx__rs_col_1" FROM ( select * from accessRights ) colQry) aliasQry)
    * teradataOffQry1 ) teradataOffQry2 WHERE teradataOffQry2.rowNum BETWEEN 0 AND
    * 50
    */
