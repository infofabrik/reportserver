...
public class RSTableToXLS {
   ...
   public void exportToExcel(RSTableModel table, Workbook workbook, Sheet sheet, boolean excel2008)
         throws ExcelExportException {
      exportToExcel(table, workbook, sheet, excel2008, null);
   }

   public void exportToExcel(RSTableModel table, Workbook workbook, Sheet sheet, boolean excel2008, TableReport report)
         throws ExcelExportException {
      if (!excel2008) {
         ...
         if (table.getColumnCount() > EXCEL_MAX_COLS)
            throw new ExcelExportException(
                  "Excel spreadsheets can't contain more than " + EXCEL_MAX_COLS + " columns."); //$NON-NLS-1$ //$NON-NLS-2$
      }
         ...
         if (table.getData().get(currentRow).isSubtotalRow()) {
            ...
            for (int column = 0; column < table.getColumnCount(); column++) {
               ...
               if (aggIndex < aggregateRowIndices.length && column == aggregateRowIndices[aggIndex]) {
                  ...
                  addSubtotal(col.getAggregateFunction(), lastGroupStart, endGroup, workbook, sheet,
                        currentRow + 1 + rowOffset, column, cell, subtotalStyles[column]);
                  aggIndex++;
            ...
            for (int column = 0; column < table.getColumnCount(); column++) {
               ...
               /* column type */
               Class<?> cType = null == styles[column] || null == content ? table.getColumnType(column)
                     : content.getClass();
      ...
      for (int column = 0; column < table.getColumnCount(); column++) {
         ...
         if (!cType.equals(java.util.Date.class) && !cType.equals(Time.class) && !cType.equals(Timestamp.class)) {
   ...
   private void addContentToCell(Class<?> cType, Object content, Workbook workbook, Sheet sheet, int column, Cell cell,
         CellStyle[] styles) throws ExcelExportException {
      /* string content */
   ...
   private void addSubtotal(AggregateFunction aggregateFunction, int startGroup, int endGroup, Workbook workbook,
         Sheet sheet, int row, int column, Cell cell, CellStyle style) {
      ...
      case COUNT_DISTINCT:
         ...
         formBuilder.append("SUMPRODUCT((").append(cStart).append(":").append(cEnd).append("<>\"\")")
               .append("/COUNTIF(").append(cStart).append(":").append(cEnd).append(",").append(cStart).append(":")
               .append(cEnd).append("&\"\"))");
         break;
      case MAX:
   ...
   private void addOveralResult(AggregateFunction aggregateFunction, List<Integer> subTotalRows, Workbook workbook,
         Sheet sheet, int column, Cell cell) {
