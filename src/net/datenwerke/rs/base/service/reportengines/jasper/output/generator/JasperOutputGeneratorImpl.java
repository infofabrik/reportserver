...
public abstract class JasperOutputGeneratorImpl implements JasperOutputGenerator {
   ...
   protected void callPostHooks(final String outputFormat, final JRAbstractExporter exporter, final JasperReport report,
         final CompiledRSJasperReport cjrReport, final User user) {

      hookHandler.getHookers(JasperExportHook.class).stream()
   ...
   protected void callPreHooks(final String outputFormat, final JRAbstractExporter exporter, final JasperReport report,
         final User user) {
      hookHandler.getHookers(JasperExportHook.class).stream()
            .filter(cb -> null != cb.getFormats() && cb.getFormats().contains(outputFormat))
