...
public class JasperPNGOutputGenerator extends JasperOutputGeneratorImpl {
   ...
   @Inject
   public JasperPNGOutputGenerator(HookHandlerService hookHandler) {
      super(hookHandler);
   }
   ...
   private BufferedImage overlayImageAsPreview(BufferedImage img) throws IOException {
      /* scale image */
      BufferedImage thumb = new BufferedImage(90, (int) (90 / (double) img.getWidth() * img.getHeight()),
            BufferedImage.TYPE_INT_RGB);
      Graphics2D g = thumb.createGraphics();

      AffineTransform affine = AffineTransform.getScaleInstance(90 / (double) img.getWidth(),
            90 / (double) img.getWidth());

      g.drawRenderedImage(img, affine);

      /* combine images */
      BufferedImage outerImg = ImageIO
            .read(this.getClass().getResource("/resources/crystalclear/Crystal_Clear_mimetype_ascii_white.png")); //$NON-NLS-1$

      BufferedImage combined = new BufferedImage(outerImg.getWidth(), outerImg.getHeight(), BufferedImage.TYPE_INT_RGB);
