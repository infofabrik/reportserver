...
public class JasperUtilsServiceImpl implements JasperUtilsService {
   ...
   @Inject
   public JasperUtilsServiceImpl(ConfigService configService, Provider<EntityManager> entityManagerProvider,
   ...
   public List<JasperParameterProposal> extractParameters(Document jrxml) {
      ...
      for (int i = 0; i < parameterTags.getLength(); i++) {
         ...
         if (null != proposal.getKey() && (proposal.getKey().trim().equals("_RS_TMP_TABLENAME")
               || proposal.getKey().trim().equals("_RS_QUERY"))) {
            // these values are set automatically. We don't want to extract them and show
            // them to the user.
            continue;
         }
