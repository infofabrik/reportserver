...
public class ConfigureBaseReportViaRequestHooker extends ConfigureReportViaRequestAndLocationImpl {
   ...
   public void adjustReport(Report report, ParameterProvider req) {
         ...
         if (report instanceof TableReport) {
            ...
            if (name.toLowerCase().equals("allcolumns") && "true".equals(req.getParameter(name))) {
               if (null != report.getTemporaryVariantType()
                  ...
                  // fetch the columns only in case of base reports
                  try {
                     columns.addAll(tableReportUtils.get().getReturnedPlainColumns((TableReport) report, null));
      ...
      if (report instanceof TableReport && !columns.isEmpty()) {
         Collections.sort(columns, (o1, o2) -> ((Integer) o1.getPosition()).compareTo(o2.getPosition()));
         ((TableReport) report).setColumns(columns);
      }

   }

   private void getHiddenCondition(Column column, int columnPosition, ParameterProvider req) {
      Enumeration<String> parameterNames = req.getParameterNames();
      while (parameterNames.hasMoreElements()) {
         String name = parameterNames.nextElement();
   ...
   private void getOrderCondition(Column column, int columnPosition, ParameterProvider req) {
      Enumeration<String> parameterNames = req.getParameterNames();
      while (parameterNames.hasMoreElements()) {
         String name = parameterNames.nextElement();
   ...
   private void getAggregateCondition(Column column, int columnPosition, ParameterProvider req) {
      Enumeration<String> parameterNames = req.getParameterNames();
      while (parameterNames.hasMoreElements()) {
         String name = parameterNames.nextElement();
