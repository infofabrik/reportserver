...
public class JasperStyleParameterParser extends JRAbstractQueryExecuter {
   ...
   public JasperStyleParameterParser(Connection connection, JRDataset dataset, Map parameters,
...
//		registerFunctions();
   ...
   public JasperStyleParameterParser(Connection connection, JRDataset dataset, Map parameters,
         ParameterSet parameterSet, boolean ignoreParameters) {
      super(getContext(), dataset, parameters);
      this.parameterMap = parameters;
      this.parameterSet = parameterSet;
      this.connection = connection;
   ...
   public JasperStyleParameterParser(Connection connection, String query, Map<String, ParameterValue> parameters,
         ParameterSet parameterSet) {
      this(connection, getDatasetFromQuery(query), createProperDataset(parameters), parameterSet);
   }

   public JasperStyleParameterParser(String query, Map<String, ParameterValue> parameters, ParameterSet parameterSet) {
      this(null, getDatasetFromQuery(query), createProperDataset(parameters), parameterSet);
   }

   public JasperStyleParameterParser(String query, Map<String, ParameterValue> parameters, ParameterSet parameterSet,
         boolean ignoreParameters) {
      this(null, getDatasetFromQuery(query), createProperDataset(parameters), parameterSet, ignoreParameters);
   }
   ...
   @SuppressWarnings("unchecked")
   private static Map createProperDataset(Map<String, ParameterValue> parameters) {
      Map map = new HashMap();

      for (String key : (Set<String>) parameters.keySet())
         map.put(key, createValueParameter(key, parameters.get(key).getValue(), parameters.get(key).getType()));
...
//		registerClauseFunction(CLAUSE_ID_NOTIN, JRSqlNotInClause.instance());
   ...
   protected void appendQueryChunk(StringBuffer sbuffer, JRQueryChunk chunk) {
      ...
      default: {
         ...
         while (m.find()) {
            ...
            parameterMap.put(group, createValueParameter(group, evaluatedExpr,
                  null != evaluatedExpr ? evaluatedExpr.getClass() : String.class));
         ...
         /*
          * juel preexecuted parameter $!{...} -> read input from temp buffer and now use
          * real buffer
          */
         ...
         while (m1.find()) {
            ...
            m1.appendReplacement(sbuffer,
                  (null != parameterSet.evaluateExpression(group) ? parameterSet.evaluateExpression(group).toString()
                        : "null"));
         }
   ...
   protected void setStatementParameter(int parameterIndex, String parameterName) throws SQLException {
      ...
      if (log.isDebugEnabled()) {
         log.debug("Parameter #" + parameterIndex + " (" + parameterName + " of type " + clazz.getName() + "): " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
               + parameterValue);
      }
   ...
   protected int setStatementMultiParameters(int parameterIndex, String parameterName) throws SQLException {
      ...
      if (paramValue.getClass().isArray()) {
         ...
         throw new JRRuntimeException("Multi parameter value is not array nor collection."); //$NON-NLS-1$
      }
   ...
   protected void setStatementMultiParameter(int parameterIndex, String parameterName, int valueIndex, Object value)
         throws SQLException {
      if (value == null) {
         throw new JRRuntimeException("Multi parameters cannot contain null values."); //$NON-NLS-1$
      }
      ...
      if (log.isDebugEnabled()) {
         log.debug("Parameter #" + parameterIndex + //$NON-NLS-1$
               " (" + parameterName + "[" + valueIndex + "] of type " + type.getName() + "): " + value); //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                                                                                                         // //$NON-NLS-4$
      }
      ...
      if (java.lang.Boolean.class.isAssignableFrom(parameterType)) {
         ...
         if (dateTimeType == Types.DATE) {
            if (parameterValue == null) {
               ...
               statement.setDate(parameterIndex, new java.sql.Date((trimmed.getTime())));
            }
            ...
            if (parameterValue == null) {
               ...
               statement.setTimestamp(parameterIndex, new Timestamp(trimmed.getTime()));
            }
            ...
            if (parameterValue == null) {
               ...
               statement.setTime(parameterIndex, new Time(trimmed.getTime()));
            }
         }
      } else {
         if (parameterValue == null) {
   ...
   public static JRValueParameter createValueParameter(final String name, final Object value, final Class<?> type) {
