...
public class Database2TableTransformer implements DataSourceDefinitionTransformer<TableDataSource> {
   ...
   @Inject
   public Database2TableTransformer(DbPoolService dbPoolService, DBHelperService dbHelper) {
      super();
   ...
   @Override
   public TableDataSource transform(DatasourceContainerProvider containerProvider, Class<?> dst,
         ParameterSet parameters) throws UnsupportedDriverException, DatabaseConnectionException {
      ...
      try {
         ...
         return new TableDBDataSource(conn, dsConfig.getQuery(), containerProvider,
               dbHelper.getDatabaseHelper(ds.getDatabaseDescriptor()));
      } catch (Exception e) {
