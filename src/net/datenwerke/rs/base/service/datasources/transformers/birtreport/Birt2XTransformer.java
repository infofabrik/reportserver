...
public abstract class Birt2XTransformer<X> implements DataSourceDefinitionTransformer<X> {
   ...
   @Inject
   public Birt2XTransformer(Provider<BirtToRSTableHelper> birtToTableProvider, DbPoolService dbPoolService,
            ...
            .build(new CacheLoader<ExtendedResultCacheKeyDatasource, CacheableResult>() {
               ...
               public CacheableResult load(ExtendedResultCacheKeyDatasource key) throws Exception {
                  ...
                  if (null == tempTableResult.getTimeout())
                     tempTableResult.setTimeout(
                           ((BirtReportDatasourceDefinition) key.getDatasource()).getDatabaseCache() * 60 * 1000L);
   ...
   public X transform(DatasourceContainerProvider containerProvider, Class<?> dst, ParameterSet parameters)
      ...
      ExtendedResultCacheKeyDatasource key = new ExtendedResultCacheKeyDatasource(container.getDatasource(), dsc,
            parameters, containerProvider, dst);
      try {
   ...
   protected TempTableResult createTempTable(DatasourceContainerProvider containerProvider, ParameterSet parameters) {
      ...
      String helperId = String.valueOf(ds.getId()) + "_" + String.valueOf(dsc.getId()) + "_"
            + String.valueOf(dsc.getOldTransientId());
      ...
      try {
         ...
         try {
            RSTableModel model = birtToTableProvider.get().asTableModel(containerProvider, report, parameters, target,
                  targetType);
