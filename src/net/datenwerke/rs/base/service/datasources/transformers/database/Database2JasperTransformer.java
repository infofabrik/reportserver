...
/**
 * Takes a
 * {@link net.datenwerke.rs.core.service.datasourcemanager.entities.DatasourceDefinition}
 * and transforms it into the corresponding JRDataSource.
 */
public class Database2JasperTransformer implements DataSourceDefinitionTransformer<JRDataSource> {
   ...
   @Inject
   public Database2JasperTransformer(DbPoolService dbPoolService) {
      super();
   ...
   public boolean consumes(DatasourceContainerProvider containerProvider, Class<?> dst) {
      ...
      return (null != container && container.getDatasource() instanceof DatabaseDatasource
            && dst.isAssignableFrom(JasperDBDataSource.class));
   }
