...
public class DatasourceParameterDefinition extends ParameterDefinition<DatasourceParameterInstance>
   ...
   @Transient
   Object getDefaultValue(DatasourceParameterService datasourceParameterService,
         ParameterSetFactory parameterSetFactory, ParameterSet parameterSet) {
      if (Mode.Single.equals(getMode())) {
         DatasourceParameterData data = getSingleDefaultValue(datasourceParameterService, parameterSetFactory,
               parameterSet);
         if (null == data)
            return null;
   ...
   protected DatasourceParameterData getSingleDefaultValue(DatasourceParameterService datasourceParameterService,
         ParameterSetFactory parameterSetFactory, ParameterSet parameterSet) {
      try {
         ...
         for (ParameterDefinition def : getAllDependents()) {
            for (ParameterInstance inst : parameterSet.getParameterList()) {
               if (null != inst.getDefinition() && null != inst.getDefinition().getKey()
                     && inst.getDefinition().getKey().equals(def.getKey())) {
                  dependees.add(inst);
                  break;
               }
            }
