...
public class DatasourceParameterServiceImpl implements DatasourceParameterService {
   ...
   @Inject
   public DatasourceParameterServiceImpl(SimpleDataSupplier simpleDataSupplyer, ParameterSetFactory parameterSetFactory,
         GroovyScriptingService scriptEngine, LicenseService licenseService, ConfigService configService) {
   ...
   @Override
   public List<DatasourceParameterData> getParameterData(final DatasourceParameterDefinition parameter,
      ...
      final GroovyScript script = !isAllowDatasourceParameterPostProcessing()
            || StringUtils.isBlank(parameter.getPostProcess()) ? null
                  : scriptEngine.compile(parameter.getPostProcess());
               ...
               if (null != script) {
                  try {
                     ...
                     throw new ReportExecutorRuntimeException(
                           BaseParameterMessages.INSTANCE.scriptPostProcessingFailed() + e.getMessage() + " ("
                                 + e.getClass().getName() + ") ",
                           e);
                  }
               } else
   ...
   public boolean isAllowDatasourceParameterPostProcessing() {
      return licenseService.isEnterprise()
            && configService.getConfigFailsafe(CONFIG_FILE).getBoolean(PARAMETER_POSTPROCESSING_ENABLE, false);
   }
