...
public class LoginServiceImpl implements LoginService {
   ...
   @Inject
   public LoginServiceImpl(LoginDao loginDao, WaitOnEventUIService waitOnEventService, UtilsUIService utilsService,
         Provider<DispatcherService> dispatcherServiceProvider, HookHandlerService hookHandler
   ...
   public void authenticate(AuthToken[] tokens, final AuthenticateCallback callback) {
      loginDao.authenticate(tokens,
            new NotamCallback<AuthenticateResultDto>(null, LoginMessages.INSTANCE.loginFailed()) { // $NON-NLS-1$
               ...
               public void doOnSuccess(AuthenticateResultDto result) {
                  ...
                  PostAuthenticateClientHook selfHookShowError = new PostAuthenticateClientHook() {
                     @Override
                     public void authenticated(final AuthenticateResultDto result,
                           final List<PostAuthenticateClientHook> chain) {
                        if (!result.isValid()) {
   ...
   public void logoff() {
      ...
      waitOnEventService.triggerEvent(REPORTSERVER_EVENT_BEFORE_LOGOUT, new CallbackOnEventDone() {
         public void execute() {
            ...
            loginDao.logoff(new ModalAsyncCallback<String>(LoginMessages.INSTANCE.loggedOut(), null, null, null, null,
                  null, null) {
               @Override
