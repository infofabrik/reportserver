...
public class EntityDiffServiceImpl implements EntityDiffService {
   ...
   public EntityDiffResult diff(Object a, Object b, EntityDiffConfig config, String guideName, boolean throwUnequal)
         throws UnequalException {
      if (null == a || null == b)
   ...
   private void compareFields(Object a, Object b, EntityDiffConfig config, String guideName, EntityDiffResult result,
         boolean throwUnequal) throws UnequalException {
   ...
   private void compareFields(Object a, Object b, EntityDiffConfig config, String guideName, Field field,
      ...
      while (null != type) {
         if (config.getFieldsToCompareBlackList().containsKey(type)
               && config.getFieldsToCompareBlackList().get(type).contains(field.getName()))
            return;
         if (config.getFieldsToCompareWhiteList().containsKey(type)
               && !config.getFieldsToCompareWhiteList().get(type).contains(field.getName()))
            return;
         if (config.getFieldsToCompareWhiteList().containsKey(type)
               && config.getFieldsToCompareWhiteList().get(type).contains(field.getName()))
            break;
   ...
   private void compareValues(Object a, Object b, Object valA, Object valB, Field field, Class<?> fieldType,
         EntityDiffConfig config, EntityDiffResult result, boolean throwUnequal) throws UnequalException {
