   ...
   public Object process(QueryByAttHandler handler, MethodInvocation invocation) {
      ...
      if (null == from || !from.isAnnotationPresent(Entity.class))
         throw new IllegalArgumentException(
               "Can only perform queries for entities: " + invocation.getMethod().getName());
      ...
      try {
         ...
         Predicate condition = queryHelper.getPredicate(invocation, fromMetadata, qb, r, metadata.type(),
               metadata.where(), providedValue);
         c.where(condition);
