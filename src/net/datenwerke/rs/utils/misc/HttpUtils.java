...
public class HttpUtils {
   ...
   public String makeContentDispositionHeader(boolean download, String filename) {
      ...
      return cd + "; filename=\"" + strictfileName + "\"; filename*=UTF-8''"
            + replaceChars(UriUtils.encode(sanitizedfilename)) + "";
   }

   /*
    * Replace the punctuation and punctuation characters and URL component
    * delimiter characters that are not replaced as shown here:
   ...
   private String replaceChars(String toEncode) {
      return toEncode.replace("-", "%2D").replace("_", "%5F").replace(".", "%2E").replace("!", "%21")
            .replace("~", "%7E").replace("*", "%2A").replace("'", "%27").replace("(", "%28").replace(")", "%29")
            .replace(";", "%3B").replace("/", "%2F").replace("?", "%3F").replace(":", "%3A").replace("&", "%26")
            .replace("=", "%3D").replace("+", "%2B").replace("$", "%24").replace(",", "%2C").replace("#", "%23");
   }
}
