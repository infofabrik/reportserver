...
public class JpaEventLoggerDaemon extends DwDaemonImpl {
   ...
   @Inject
   public JpaEventLoggerDaemon(@EventLoggerCheckInterval Long checkInterval,
         Provider<EntityManager> entityManagerProvider, Provider<UnitOfWork> unitOfWorkProvider,
   ...
   private AuditLogEntry getLogEntry(LoggedEvent event) {
      ...
      entry.setLogProperties(event.getLoggedProperties().entrySet().stream()
            .map(e -> new LogProperty(e.getKey(), e.getValue())).collect(toSet()));
