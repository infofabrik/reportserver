...
public class ParameterRpcServiceImpl extends SecuredRemoteServiceServlet implements ParameterRpcService {
   ...
   @Inject
   public ParameterRpcServiceImpl(EntityClonerService entityCloner, ReportService reportManager,
         ReportParameterService parameterService, DtoService dtoGenerator, SecurityService securityService) {
   ...
   @Override
   public ReportDto addParameter(ParameterDefinitionDto parameter, @Named("node") AbstractNodeDto correspondingNode)
      ...
      if (!(correspondingNode instanceof ReportDto) && !(correspondingNode instanceof ReportFolderDto))
         throw new IllegalArgumentException("Parameters can only be added to reports or folders. " //$NON-NLS-1$
               + correspondingNode.getClass().getName() + " was passed."); //$NON-NLS-1$
   ...
   @Transactional(rollbackOn = { Exception.class })
   @Override
   public ReportDto updateParameterInstances(Collection<ParameterDefinitionDto> parameterDtos)
         throws ServerCallFailedException {
   ...
   @Transactional(rollbackOn = { Exception.class })
   @Override
   public ReportDto removeParameters(Collection<ParameterDefinitionDto> parameterDtos)
         throws ServerCallFailedException {
      Collection<ParameterDefinition> parameters = getParametersAndCheckRights(parameterDtos);
   ...
   private Collection<ParameterDefinition> getParametersAndCheckRights(Collection<ParameterDefinitionDto> parameterDtos)
   ...
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public ReportDto duplicateParameters(List<ParameterDefinitionDto> parameterDtos, AbstractNodeDto correspondingNode)
         throws ServerCallFailedException, ExpectedException {
      if (null == parameterDtos || parameterDtos.isEmpty())
         return null;
