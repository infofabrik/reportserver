...
public class EnvironmentAfterStartupInformation implements LateInitHook {
   ...
   @Inject
   public EnvironmentAfterStartupInformation(ApplicationPropertiesService propertiesService,
         Provider<Set<ReportServerPAM>> pamProvider, Provider<ServletContext> servletContextProvider) {
   ...
   private void applicationServerConfig(StringBuilder sb) {
      try {
         sb.append("Application Server: ").append(servletContextProvider.get().getServerInfo()).append("\r\n\r\n");
      } catch (Exception e) {
