...
public class ParameterSet implements Iterable<ParameterInstance> {
   ...
   public ParameterSet(@Nullable @Assisted User user) {
      this.user = user;
      this.report = null;
   }

   public ParameterSet(AuthenticatorService authService) {
      this.user = authService.getCurrentUser();
      this.report = null;
   }

   public ParameterSet(AuthenticatorService authService, @Nullable @Assisted User user,
         @Nullable @Assisted Report report) {
      this.user = null == user ? authService.getCurrentUser() : user;
      this.report = report;
   }

   public ParameterSet(AuthenticatorService authService, @Nullable @Assisted Report report) {
      this.user = authService.getCurrentUser();
      this.report = report;
   }
      ...
      for (Entry<String, Object> entry : variableMap.entrySet())
         if (null != entry.getValue())
            parameterMap.put(entry.getKey(),
                  new ParameterValueImpl(entry.getKey(), entry.getValue(), entry.getValue().getClass()));
      ...
      for (ParameterInstance parameterInstance : parameters) {
         if (parameterInstance.getKey().equals(key)) {
            ...
            if (parameterDefinition instanceof DateTimeParameterDefinition) {
               ...
               switch (paramMode) {
               case Date:
                  return DateFormat.getDateInstance(DateFormat.SHORT, localizationServiceProvider.get().getLocale())
                        .format((Date) dateValue);
               case Time:
                  return DateFormat.getTimeInstance(DateFormat.SHORT, localizationServiceProvider.get().getLocale())
                        .format((Date) dateValue);
               case DateTime:
   ...
   public Object evaluateExpression(String text) {
      ...
      for (ParameterSetReplacementProvider repProvider : replacementProviderProvider.get()) {
         ...
         if (null != replacements)
            for (Entry<String, ParameterValue> entry : replacements.entrySet())
               vm.setVariable(entry.getKey(),
                     factory.createValueExpression(entry.getValue().getValue(), entry.getValue().getType()));
      ...
      /* store variables */
      for (Entry<String, Object> entry : variableMap.entrySet())
         vm.setVariable(entry.getKey(), factory.createValueExpression(entry.getValue(),
               (null != entry.getValue() ? entry.getValue().getClass() : Object.class)));
