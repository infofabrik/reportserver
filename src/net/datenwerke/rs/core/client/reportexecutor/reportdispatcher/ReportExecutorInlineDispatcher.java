...
public class ReportExecutorInlineDispatcher extends DispatcherTakeOverHookImpl {
   ...
   protected boolean checkParameters(HistoryLocation hLocation) {
      return hLocation.hasParameter(PARAM_ID) || hLocation.hasParameter(PARAM_KEY) || hLocation.hasParameter(PARAM_UUID)
            || hLocation.hasParameter(PARAM_PATH);
   }
   ...
   protected void displayFull(HistoryLocation hLocation, final ReportDto result, Viewport vp) {
      final String defaultView = hLocation.hasParameter(DEFAULT_VIEW) ? hLocation.getParameterValue(DEFAULT_VIEW)
            : AbstractReportPreviewView.VIEW_ID;
