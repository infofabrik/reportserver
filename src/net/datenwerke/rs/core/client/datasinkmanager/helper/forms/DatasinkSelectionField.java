...
public class DatasinkSelectionField implements HasValueChangeHandlers<DatasinkContainerDto> {
	...
	public HasValueFieldBinding initFormBinding(final DatasinkContainerProviderDto datasinkContainerProvider) {
		...
		HasValueFieldBinding binding = new HasValueFieldBinding(dsField, datasinkContainer, DatasinkContainerDtoPA.INSTANCE.datasink()){
			@Override  
	...
	protected void updateFieldsAndModel(DatasinkDefinitionDto datasink, final DatasinkContainerProviderDto datasinkContainerProvider) {
		...
		/* if the new datasink equals the old one (type) and configurator does not want to reload then do nothing */
		DatasinkDefinitionConfigConfigurator tempConfigurator = datasinkService.getConfigurator(datasink.getClass());
		if(null != oldDatasink && datasink.getClass().equals(oldDatasink.getClass()) && ! tempConfigurator.isReloadOnDatasinkChange())
			return;
		else
	...
	public HandlerRegistration addValueChangeHandler(
			ValueChangeHandler<DatasinkContainerDto> handler) {
