...
public class UrlViewUiStartup {
   ...
   @Inject
   public UrlViewUiStartup(final WaitOnEventUIService waitOnEventService, final SecurityUIService securityService,
         final HookHandlerService hookHandler, final Provider<LoginService> loginServiceProvider,
         final UrlViewDao dao) {

      waitOnEventService.permanentCallbackOnEvent(LoginService.REPORTSERVER_EVENT_AFTER_ANY_LOGIN,
                     ...
                     public void onSuccess(Map<String, Map<String, List<String[]>>> result) {
                        try {
                           if (result.containsKey("adminviews"))
                              hookHandler.attachHooker(MainPanelViewProviderHook.class,
                                    new ViewProviderHooker(hookHandler, loginServiceProvider, result.get("adminviews")),
                                    HookHandlerService.PRIORITY_LOWER);

                           if (result.containsKey("objectinfo"))
                              hookHandler.attachHooker(ObjectPreviewTabProviderHook.class,
                                    new ObjectInfoProvider(hookHandler, loginServiceProvider, result.get("objectinfo")),
                                    HookHandlerService.PRIORITY_LOWER);
                           ...
                           if (result.containsKey("module")) {
                              ...
                              for (List<String[]> v : map.values()) {
                                 ...
                                 String url = values[1].replace("${username}",
                                       loginServiceProvider.get().getCurrentUser().getUsername());
                                 ;

                                 hookHandler.attachHooker(ClientMainModuleProviderHook.class,
                                       new ClientMainModuleProviderHook(
                                             new UrlViewClientMainModule(hookHandler, name, url)),
                                       priority);
                              }
                           }
