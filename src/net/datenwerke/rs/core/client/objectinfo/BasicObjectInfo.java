...
public class BasicObjectInfo implements ObjectPreviewTabKeyInfoProvider {
   ...
   protected void fillTable(FlexTable table, AbstractNodeDto node) {
      ...
      HTML name = new HTML(new SafeHtmlBuilder().appendHtmlConstant("<b>").appendEscaped(BaseMessages.INSTANCE.name())
            .appendEscaped(":").appendHtmlConstant("</b>").toSafeHtml());
      name.setWordWrap(false);
      table.setWidget(cnt, 0, name);
      table.setText(cnt++, 1, node.toDisplayTitle());

      HTML description = new HTML(
            new SafeHtmlBuilder().appendHtmlConstant("<b>").appendEscaped(BaseMessages.INSTANCE.description())
                  .appendEscaped(":").appendHtmlConstant("</b>").toSafeHtml());
      description.setWordWrap(false);
      table.setWidget(cnt, 0, description);
      table.setText(cnt++, 1, ((AbstractNodeDtoDec) node).getDescription());

      HTML createdOn = new HTML(
            new SafeHtmlBuilder().appendHtmlConstant("<b>").appendEscaped(BaseMessages.INSTANCE.createdOn())
                  .appendEscaped(":").appendHtmlConstant("</b>").toSafeHtml());
      ...
      HTML type = new HTML(new SafeHtmlBuilder().appendHtmlConstant("<b>").appendEscaped(BaseMessages.INSTANCE.type())
            .appendEscaped(":").appendHtmlConstant("</b>").toSafeHtml());
      type.setWordWrap(false);
      table.setWidget(cnt, 0, type);
      table.setText(cnt++, 1, node.toTypeDescription());

      HTML id = new HTML(new SafeHtmlBuilder().appendHtmlConstant("<b>").appendEscaped(BaseMessages.INSTANCE.id())
            .appendEscaped(":").appendHtmlConstant("</b>").toSafeHtml());
