...
public class ParameterView extends MainPanelView {
   ...
   @Inject
   public ParameterView(ClipboardUiService clipboardService, ParameterUIService parameterService,
         ToolbarService toolbarService, ParameterDao parameterDao) {
   ...
   protected ToolBar createToolbar() {
      ...
      /* add parameter */
      DwTextButton addParamBtn = toolbarService.createSmallButtonLeft(ParametersMessages.INSTANCE.parameter(),
            BaseIcon.COG_ADD);
   ...
   protected void prepareRemove() {
      ...
      if (parameterGrid.getSelectionModel().getSelectedItems().size() == 1)
         cmb = new DwConfirmMessageBox(BaseMessages.INSTANCE.remove(),
               ParametersMessages.INSTANCE.reallyRemoveParameterText());
      else
         cmb = new DwConfirmMessageBox(BaseMessages.INSTANCE.remove(), ParametersMessages.INSTANCE
               .reallyRemoveParametersText(parameterGrid.getSelectionModel().getSelectedItems().size()));
   ...
   protected Grid<ParameterDefinitionDto> createGrid() {
      ...
      ColumnConfig<ParameterDefinitionDto, ParameterDefinitionDto> iconConfig = new ColumnConfig<ParameterDefinitionDto, ParameterDefinitionDto>(
            new IdentityValueProvider<ParameterDefinitionDto>("__parameter_icon"), 30);
      iconConfig.setMenuDisabled(true);
      iconConfig.setCell(new AbstractCell<ParameterDefinitionDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, ParameterDefinitionDto value,
               SafeHtmlBuilder sb) {
      ...
      /* key column */
      ColumnConfig<ParameterDefinitionDto, String> keyConfig = new ColumnConfig<ParameterDefinitionDto, String>(
            defPa.key(), 150, reportManagerMessages.key());
      keyConfig.setMenuDisabled(true);
      keyConfig.setSortable(false);
      columns.add(keyConfig);

      /* name column */
      ColumnConfig<ParameterDefinitionDto, String> nameConfig = new ColumnConfig<ParameterDefinitionDto, String>(
            defPa.name(), 200, BaseMessages.INSTANCE.name());
      nameConfig.setMenuDisabled(true);
      nameConfig.setSortable(false);
      columns.add(nameConfig);

      /* type column */
      ColumnConfig<ParameterDefinitionDto, String> ccType = new ColumnConfig<ParameterDefinitionDto, String>(
            new DisplayTitleValueProvider<ParameterDefinitionDto>(), 150, reportManagerMessages.type());
      ccType.setMenuDisabled(true);
      ccType.setSortable(false);
      columns.add(ccType);

      /* config indicator column */
      ColumnConfig<ParameterDefinitionDto, ParameterDefinitionDto> ccConfigIndicator = new ColumnConfig<ParameterDefinitionDto, ParameterDefinitionDto>(
            new IdentityValueProvider<ParameterDefinitionDto>("__parameter_mods"), 120,
            ParametersMessages.INSTANCE.optionsHeader());
      ...
      /* create grid */
      final Grid<ParameterDefinitionDto> grid = new Grid<ParameterDefinitionDto>(parameterStore,
            new ColumnModel<ParameterDefinitionDto>(columns));
      ...
      editing.addCompleteEditHandler(new CompleteEditHandler<ParameterDefinitionDto>() {
         @Override
         public void onCompleteEdit(CompleteEditEvent<ParameterDefinitionDto> event) {
   ...
   protected void displayEditParameter(final ParameterDefinitionDto definition) {
      ...
      editParameterWindow.setHeading(
            ParametersMessages.INSTANCE.editParameter() + (definition.getKey() == null ? "key" : definition.getKey())); //$NON-NLS-1$
      ...
      editParameterWindow.addCard(ParametersMessages.INSTANCE.parameterProperties(), BaseIcon.INFO_CIRCLE,
            mainOptionsComponent);

      /* parameter special options pane */
      Widget editComponent = createParameterSpecialOptionsPanel(definition);
      editParameterWindow.addCard(ParametersMessages.INSTANCE.specialProperties(), BaseIcon.COG, editComponent);
   ...
   protected Widget createParameterMainOptionsPanel(final ParameterDefinitionDto definition) {
      ...
      form.beginRow();
      form.addField(String.class, ParameterDefinitionDtoPA.INSTANCE.name(), BaseMessages.INSTANCE.propertyName()); // $NON-NLS-1$
                                                                                                                   // //$NON-NLS-2$
      form.addField(String.class, ParameterDefinitionDtoPA.INSTANCE.key(), ParametersMessages.INSTANCE.key());
      form.endRow();

      form.addField(String.class, ParameterDefinitionDtoPA.INSTANCE.description(),
            BaseMessages.INSTANCE.propertyDescription(), new SFFCTextAreaImpl());

      if (configurator.canDependOnParameters()) {
         form.addField(ParameterDefinitionDto.class, ParameterDefinitionDtoPA.INSTANCE.dependsOn(),
               ParametersMessages.INSTANCE.dependsOn(), new SFFCBaseModel<ParameterDefinitionDto>() {
      ...
      form.beginRow();
      form.addField(Boolean.class, ParameterDefinitionDtoPA.INSTANCE.hidden(),
            ParametersMessages.INSTANCE.propertyHidden());
      form.addField(Boolean.class, ParameterDefinitionDtoPA.INSTANCE.editable(),
            ParametersMessages.INSTANCE.propertyEditable());
      form.addField(Boolean.class, ParameterDefinitionDtoPA.INSTANCE.mandatory(),
            ParametersMessages.INSTANCE.propertyMandatory());
      form.addField(Boolean.class, ParameterDefinitionDtoPA.INSTANCE.displayInline(),
            ParametersMessages.INSTANCE.propertyDisplayInline());
      form.endRow();

      form.setLabelAlign(LabelAlign.TOP);
      form.setFieldWidth(150);
      form.addField(Integer.class, ParameterDefinitionDtoPA.INSTANCE.labelWidth(),
            ParametersMessages.INSTANCE.propertyLabelWidth());

      form.bind(definition);

      DwContentPanel panel = new DwContentPanel();
      panel.setLightHeader();
      panel.setHeading(ParametersMessages.INSTANCE.parameterProperties() + " ("
            + ParametersMessages.INSTANCE.generalPoperties() + ")");
   ...
   protected Widget createParameterSpecialOptionsPanel(ParameterDefinitionDto definition) {
      ...
      wrapper.setHeading(ParametersMessages.INSTANCE.parameterProperties() + " ("
   ...
   public void addParameter(ParameterConfigurator configurator, AbstractNodeDto node) {
      if (!(node instanceof ReportDto))
         throw new IllegalArgumentException(
               "Parameters can only be added to reports. " + node.getClass().getName() + " was passed."); //$NON-NLS-1$ //$NON-NLS-2$

      /* call server */
      parameterDao.addParameter((ReportDto) getSelectedNode(), configurator.getNewDto((ReportDto) node), node,
            new NotamCallback<ReportDto>(ParametersMessages.INSTANCE.parameterAdded()) { // $NON-NLS-1$
               @Override
   ...
   protected void updateParameter(final ParameterDefinitionDto pd) {
      ...
      parameterDao.updateParameter((ReportDto) getSelectedNode(), pd,
            new NotamCallback<ReportDto>(ParametersMessages.INSTANCE.parameterChanged()) {
               @Override
   ...
   protected void removeParameters(Collection<ParameterDefinitionDto> parameters) {
      ...
      if (null != parameters && !parameters.isEmpty()) {
         parameterDao.removeParameters((ReportDto) getSelectedNode(), parameters,
               new NotamCallback<ReportDto>(ParametersMessages.INSTANCE.parameterRemoved()) {
                  @Override
                  public void doOnSuccess(ReportDto adjustedReport) {
                     updateStore(adjustedReport);
                  }
   ...
   protected void moveParameter(ParameterDefinitionDto parameter, int to) {
      /* call server */
      parameterDao.movedParameter(parameter, to,
            new NotamCallback<ReportDto>(ParametersMessages.INSTANCE.parameterMoved()) {
               @Override
               public void doOnSuccess(ReportDto updatedNode) {
                  updateStore(updatedNode);
               }
   ...
   protected void updateParameterInstances(final List<ParameterDefinitionDto> models) {
      if (null == models || models.isEmpty()) {
         new DwAlertMessageBox(BaseMessages.INSTANCE.warning(), ParametersMessages.INSTANCE.noParametersSelected())
               .show();
         return;
      }
   ...
   public void updateStore(AbstractNodeDto node) {
      if (!(node instanceof ReportDto) && !(node instanceof ReportFolderDto))
         throw new IllegalArgumentException(
               "Parameters can only be added to reports or folders. " + node.getClass().getName() + " was passed."); //$NON-NLS-1$ //$NON-NLS-2$
