...
public class DatasourceSelectionField implements HasValueChangeHandlers<DatasourceContainerDto> {
   ...
   private void showFailureMessageBox() {
      MessageBox msg = new MessageBox(DatasourcesMessages.INSTANCE.useDefaultFailureTitle(),
            DatasourcesMessages.INSTANCE.useDefaultFailureMessage());
      msg.show();
   }
   ...
   public void addDisplayDefaultButton() {
      ...
      dsField.addTwinTriggerClickHandler(event -> {
         datasourceDao.getDefaultDatasource(
               new NotamCallback<DatasourceDefinitionDto>(DatasourcesMessages.INSTANCE.useDefaultSuccessMessage(),
                     DatasourcesMessages.INSTANCE.useDefaultFailureMessage()) {
                  @Override
   ...
   public HasValueFieldBinding initFormBinding(final DatasourceContainerProviderDto datasourceContainerProvider) {
      ...
      HasValueFieldBinding binding = new HasValueFieldBinding(dsField, datasourceContainer,
            DatasourceContainerDtoPA.INSTANCE.datasource()) {
         @Override
   ...
   protected void updateFieldsAndModel(DatasourceDefinitionDto datasource,
      ...
      /*
       * if the new datasource equals the old one (type) and configurator does not
       * want to reload then do nothing
       */
      DatasourceDefinitionConfigConfigurator tempConfigurator = datasourceService
            .getConfigurator(datasource.getClass());
      if (null != oldDatasource && datasource.getClass().equals(oldDatasource.getClass())
            && !tempConfigurator.isReloadOnDatasourceChange())
         return;
      else
      ...
      boolean configuratorConsumesDatasourceAndConfig = datasourceConfigConfigurator
            .consumes(datasourceContainer.getDatasource(), datasourceContainer.getDatasourceConfig());
      ...
      Iterable<Widget> dsDefaultAdditionalConfigFields = datasourceConfigConfigurator
            .getDefaultAdditionalFormfields(datasourceConfig, datasource, this, datasourceContainerProvider);

      /*
       * we might be able to exit here, if we are not supposed to display the
       * configurations special fields.
       */
      ...
      Iterable<Widget> dsOptionalAdditionalConfigFields = (displayOptionalAdditionalConfigFields
            ? datasourceConfigConfigurator.getOptionalAdditionalFormfields(datasourceConfig, datasource, this,
      ...
      Integer index = null;
      /*
       * we first have to display the specialized fields, and can only then update the
       * fields
       */
      if (allAdditionalConfigFields.iterator().hasNext()) {
         ...
         if (allAdditionalConfigFields instanceof SimpleForm) {
            ...
            for (Widget field : allAdditionalConfigFields) {
               ...
               else if (field instanceof FieldLabel && null != ((FieldLabel) field).getWidget()
                     && ((FieldLabel) field).getWidget() instanceof Component
                     && !((Component) ((FieldLabel) field).getWidget()).isAutoWidth())
                  configContainer.add(field, new VerticalLayoutData(-1, -1, new Margins(0, 0, 10, 0)));
               else
                  configContainer.add(field, new VerticalLayoutData(1, -1, new Margins(0, 0, 10, 0)));
            }
   ...
   public void setSpecificDatasourceConfigs(Set<SpecificDatasourceConfig> specificDatasourceConfigs) {
   ...
   @Override
   public HandlerRegistration addValueChangeHandler(ValueChangeHandler<DatasourceContainerDto> handler) {
