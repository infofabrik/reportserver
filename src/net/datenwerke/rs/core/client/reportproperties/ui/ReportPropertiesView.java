...
public class ReportPropertiesView extends MainPanelView {
	...
	private Component createInheritedPropertiesGrid() {
		...
		inheritedPropertiesStore.addSortInfo(
				new StoreSortInfo<>(ReportStringPropertyDtoPA.INSTANCE.name(), SortDir.ASC));
		...
		ColumnConfig<ReportStringPropertyDto, String> valueConfig = new ColumnConfig<>(
				ReportStringPropertyDtoPA.INSTANCE.strValue(), 300,
				ReportPropertiesMessages.INSTANCE.gridValueHeader());
		valueConfig.setMenuDisabled(true);

		columns.add(valueConfig);

		/* create grid */
		inheritedPropertiesGrid = new Grid<ReportStringPropertyDto>(inheritedPropertiesStore,
	...
	private void createGrid() {
		...
		/* icon column */
		ColumnConfig<ReportStringPropertyDto,ReportStringPropertyDto> icConfig = new ColumnConfig<>(new IdentityValueProvider<ReportStringPropertyDto>(), 40); 
		icConfig.setMenuDisabled(true);
		icConfig.setCell(new AbstractCell<ReportStringPropertyDto>(){
			@Override
			public void render(com.google.gwt.cell.client.Cell.Context context, ReportStringPropertyDto value,
