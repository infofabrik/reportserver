...
public class ScheduleAsBoxFileEmailNotificationHooker extends SchedulerExecutionHookAdapter {
   ...
   public static final String PROPERTY_BOX_NOTIFICATION_SUBJECT_SUCCESS = "scheduler.fileactionBox.subject";
   public static final String PROPERTY_BOX_NOTIFICATION_TEXT_SUCCESS = "scheduler.fileactionBox.text";

   private static final String PROPERTY_BOX_NOTIFICATION_DISABLED = "scheduler.fileactionBox[@disabled]";
   private static final String PROPERTY_BOX_NOTIFICATION_HTML = "scheduler.fileactionBox[@html]";
   ...
   @Inject
   public ScheduleAsBoxFileEmailNotificationHooker(@SchedulerModuleProperties Configuration config,
   ...
   public void actionExecutionEndedSuccessfully(AbstractJob abstractJob, AbstractAction abstractAction,
      ...
      if (!(abstractAction instanceof ScheduleAsBoxFileAction))
         return;

      ReportExecuteJob job = (ReportExecuteJob) abstractJob;

      try {
         sendmail((ScheduleAsBoxFileAction) abstractAction, job, null);
   ...
   private void sendmail(ScheduleAsBoxFileAction action, ReportExecuteJob job, Exception e) throws MessagingException {
      if (config.getBoolean(PROPERTY_BOX_NOTIFICATION_DISABLED, false))
         return;
      ...
      datamap.put(PROPERTY_OWNERS, UserListForJuelPrinter.createInstance(new ArrayList<>(job.getOwners()),
            config.getBoolean(PROPERTY_BOX_NOTIFICATION_HTML, false)));
      ...
      String subjectTemplate = config.getString(PROPERTY_BOX_NOTIFICATION_SUBJECT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionBoxMsgSubject"));
      String messageTemplate = config.getString(PROPERTY_BOX_NOTIFICATION_TEXT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionBoxMsgText"));

      MailTemplate mailTemplate = new MailTemplate();
      mailTemplate.setHtml(config.getBoolean(PROPERTY_BOX_NOTIFICATION_HTML, false));
