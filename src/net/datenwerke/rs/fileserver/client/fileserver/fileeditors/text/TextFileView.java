...
public class TextFileView extends SimpleFormView {
   ...
   protected void configureSimpleForm(final SimpleForm form) {
      form.setHeading(FileServerMessages.INSTANCE.fileViewHeader()
      ...
      final String codeMirrorField = form.addField(String.class, StringBaseModelPa.INSTANCE.value(),
            new SFFCCodeMirror() {
   ...
   protected void onSubmit(SimpleFormSubmissionCallback callback) {
      mask(BaseMessages.INSTANCE.loadingMsg());
      fileServerDao.updateFile((FileServerFileDto) getSelectedNode(), bindingEntity.getValue(),
            new NotamCallback<Void>(BaseMessages.INSTANCE.changesApplied()) {
               @Override
