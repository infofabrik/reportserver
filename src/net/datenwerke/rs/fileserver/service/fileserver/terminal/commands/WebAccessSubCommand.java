...
public class WebAccessSubCommand implements DirModCommandHook {
	...
	public CommandResult execute(CommandParser parser, TerminalSession session) throws ObjectResolverException {
		...
		String webAccess = arguments.get(1);
		...
		Collection<Object> resolvedObjects = session.getObjectResolver().getObjects(path, Read.class);
		List<?> searchResults = (List<?>) resolvedObjects;

		if (0 != resolvedObjects.size()) {
			for (Object item : searchResults) {
				if (!(item instanceof FileServerFolder))
			...
			for (Object item : searchResults) {
				...
				try {
					List<HistoryLink> links = historyService.buildLinksFor(item);
					for (HistoryLink link : links) {
						cr.addResultHyperLink(link.getObjectCaption(), link.getLink());
					}
