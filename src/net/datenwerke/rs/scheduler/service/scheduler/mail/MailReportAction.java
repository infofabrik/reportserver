...
public class MailReportAction extends AbstractAction {
   ...
   @Transient
   @Inject
   private static Provider<MailMessages> messages = LocalizationServiceImpl.getMessagesProvider(MailMessages.class);
   ...
   @Transient
   @Inject
   private String attachementNameTemplate;
   ...
   private SimpleAttachment prepareAttachment(ReportExecuteJob job, String filenamePrefix)
      ...
      if (compressed) {
         ...
         return new SimpleAttachment(os.toByteArray(), "application/zip", filenamePrefix + ".zip" //$NON-NLS-2$
         );
      } else {
         return new SimpleAttachment(job.getExecutedReport().getReport(), job.getExecutedReport().getMimeType(),
