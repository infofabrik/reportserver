...
public class SchedulerExportConfigurationForm extends DwContentPanel implements Validatable, WizardAware, WizardResizer, WizardPageChangeListener {
	...
	protected void initForm(final ReportDto report, final Collection<ReportViewConfiguration> configs, 
			final ArrayList<SendToClientConfig> sendToConfigs, final ReportScheduleDefinition definition) {
		setBorders(false);
		setBodyBorder(false);
		setHeaderVisible(false);
		enableScrollContainer();

		final SimpleMultiForm form = SimpleMultiForm.createInlineInstance();
	...
	public void configureConfig(final ReportScheduleDefinition configDto) {
		snippetProviders
			.stream()
			.filter(hooker -> formMap.containsKey(hooker))
	...
	public void onPageChange(int pageNr, Widget page) {
		...
		for(ScheduleExportSnippetProviderHook hooker : snippetProviders){
			...
			final SimpleForm xform = formMap.get(hooker);
			hooker.onWizardPageChange(pageNr, page, xform, definition, report);
		}
