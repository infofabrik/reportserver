...
public class SchedulerUiServiceImpl implements SchedulerUiService {
   ...
   private static ReportScheduleJobListInformationPA scheduleInfoPa = GWT
   ...
   @Override
   public ColumnModel<ReportScheduleJobListInformation> createReportScheduleListColumnModel(boolean displayJobId,
         boolean displayExecutorColumn, boolean displayScheduledByColumn) {
      List<ColumnConfig<ReportScheduleJobListInformation, ?>> colConfig = new ArrayList<ColumnConfig<ReportScheduleJobListInformation, ?>>();

      if (displayJobId) {
         ColumnConfig<ReportScheduleJobListInformation, Long> idColumn = new ColumnConfig<ReportScheduleJobListInformation, Long>(
               scheduleInfoPa.jobId(), 80, SchedulerMessages.INSTANCE.jobId());
         idColumn.setMenuDisabled(true);
         colConfig.add(idColumn);
      }

      ColumnConfig<ReportScheduleJobListInformation, Long> idColumn = new ColumnConfig<ReportScheduleJobListInformation, Long>(
            scheduleInfoPa.reportId(), 80, SchedulerMessages.INSTANCE.gridIdLabel());
      idColumn.setMenuDisabled(true);
      colConfig.add(idColumn);

      ColumnConfig<ReportScheduleJobListInformation, ReportScheduleJobListInformation> descriptionConfig = new ColumnConfig<ReportScheduleJobListInformation, ReportScheduleJobListInformation>(
            new IdentityValueProvider<ReportScheduleJobListInformation>(scheduleInfoPa.jobTitle().getPath()), 270,
            SchedulerMessages.INSTANCE.jobTitle());
      descriptionConfig.setCell(new AbstractCell<ReportScheduleJobListInformation>() {
         ...
         public void render(com.google.gwt.cell.client.Cell.Context context, ReportScheduleJobListInformation value,
            ...
            String description = null == value.getJobDescription() ? SchedulerMessages.INSTANCE.noDescription()
                  : Format.ellipse(value.getJobDescription(), 270);
            sb.appendHtmlConstant("<div class=\"rs-list-name-desc\"><div class=\"title\">" + header
      ...
      ColumnConfig<ReportScheduleJobListInformation, Date> lastColumn = new ColumnConfig<ReportScheduleJobListInformation, Date>(
            scheduleInfoPa.lastScheduled(), 128, SchedulerMessages.INSTANCE.gridLastScheduled());
      ...
      ColumnConfig<ReportScheduleJobListInformation, Date> nextColumn = new ColumnConfig<ReportScheduleJobListInformation, Date>(
            scheduleInfoPa.nextScheduled(), 128, SchedulerMessages.INSTANCE.gridNextSchedule());
