...
public class SchedulerRpcServiceImpl extends SecuredRemoteServiceServlet implements SchedulerRpcService {
   ...
   @SecurityChecked(genericTargetVerification = {
         @GenericTargetVerification(target = SchedulingBasicSecurityTarget.class, verify = @RightsVerification(rights = Execute.class)) })
   @Override
   ...
   protected ReportExecuteJob doSchedule(final ReportScheduleDefinition scheduleDTO, final ReportExecuteJob previous)
         ...
         try {
            ...
            if (!(executorNode instanceof User))
               throw new ExpectedException(
                     "User could not be loaded. Not a user. Id: " + scheduleDTO.getExecutor().getId());
      ...
      securityService.assertRights(owners, report, Execute.class);
      ...
      try {
         reportExecutorService.isExecutable(report, scheduleDTO.getOutputFormat(),
               configList.toArray(new ReportExecutionConfig[] {}));
      ...
      /* create job */
      final ReportExecuteJob job = reportExecuteJobProvider.get();
      ...
      for (Long id : scheduleDTO.getRecipients()) {
         try {
            ...
            if (recipient instanceof User
                  && (null == ((User) recipient).getEmail() || "".equals(((User) recipient).getEmail())))
               throw new ExpectedException(SchedulerMessages.INSTANCE.errorNoEmail() + " "
                     + ((User) recipient).getFirstname() + " " + ((User) recipient).getLastname());
   ...
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public void clearErrorStatus(Long jobId) throws ServerCallFailedException {
      AbstractJob job = schedulerService.getJobById(jobId);
      if (!(job instanceof ReportExecuteJob))
         throw new IllegalArgumentException("Expected " + ReportExecuteJob.class);
   ...
   @SecurityChecked(genericTargetVerification = {
         @GenericTargetVerification(target = SchedulingAdminSecurityTarget.class, verify = @RightsVerification(rights = Execute.class)) })
   @Override
   @Transactional(rollbackOn = { Exception.class })
   ...
   @SecurityChecked(genericTargetVerification = {
         @GenericTargetVerification(target = SchedulingBasicSecurityTarget.class, verify = @RightsVerification(rights = Read.class)) })
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public PagingLoadResult<ReportScheduleJobListInformation> getReportJobList(
         JobFilterConfigurationDto jobFilterConfigDto, List<JobFilterCriteriaDto> addCriterionsDto)
         throws ServerCallFailedException {
      /* convert filter */
      ...
      if (filter instanceof ReportServerJobFilter) {
         ...
         if (!addCriteria.isEmpty()) {
            for (JobFilterCriteria ac : addCriteria) {
               if (ac instanceof AuthoritativeSchedulerCriteria
                     && ((AuthoritativeSchedulerCriteria) ac).isAuthoritative()) {
                  allow = true;
                  break;
               }
            }
         }
         if (!allow && ((ReportServerJobFilter) filter).isAnyUser()
               && !securityService.checkRights(SchedulingAdminSecurityTarget.class, Read.class)) {
            return new PagingLoadResultBean<ReportScheduleJobListInformation>(
                  new ArrayList<ReportScheduleJobListInformation>(), 0, 0);
         }
      }
      ...
      if (!securityService.checkRights(SchedulingAdminSecurityTarget.class, Read.class)) {
         if (!((ReportServerJobFilter) filter).isToCurrentUser()
               && !((ReportServerJobFilter) filter).isFromCurrentUser())
      ...
      return new PagingLoadResultBean<ReportScheduleJobListInformation>(jobInfoList, (int) maxResults,
            jobFilterConfigDto.getOffset());
   }
   ...
   @Override
   @Transactional(rollbackOn = { Exception.class })
   public ReportScheduleJobInformation loadFullScheduleInformation(ReportScheduleJobListInformation selected)
         throws ServerCallFailedException {
      return loadScheduleInformation(selected, 5);
   }

   protected ReportScheduleJobInformation loadScheduleInformation(ReportScheduleJobListInformation selected,
         Integer maxNumberOfHistEntries) throws ServerCallFailedException {
      ...
      final ReportExecuteJob job = (ReportExecuteJob) aJob;
      ...
      /* schedule definition */
      final ReportScheduleDefinition rsd = new ReportScheduleDefinition();
      ...
      for (int i = 0, l = null == maxNumberOfHistEntries ? entries.size()
            : Math.min(maxNumberOfHistEntries, entries.size()); i < l; i++) {
         ...
         if (Outcome.FAILURE == entry.getOutcome())
            dto.setBadErrorDescription(null == entry.getBadErrorDescription() ? ""
                  : entry.getBadErrorDescription().substring(0,
                        Math.min(100, entry.getBadErrorDescription().length())));
   ...
   @SecurityChecked(genericTargetVerification = {
         @GenericTargetVerification(target = SchedulingBasicSecurityTarget.class, verify = @RightsVerification(rights = Read.class)) })
   @Override
   public ReportScheduleJobInformation loadDetailsFor(ReportScheduleJobListInformation selected)
         throws ServerCallFailedException {
   ...
   public boolean isDefaultEmailCompression() throws ServerCallFailedException {
      return configServiceProvider.get().getConfigFailsafe("main/main.cf")
