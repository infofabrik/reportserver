...
public class ScheduleAsGoogleDriveFileEmailNotificationHooker extends SchedulerExecutionHookAdapter {
   ...
   public static final String PROPERTY_GOOGLEDRIVE_NOTIFICATION_SUBJECT_SUCCESS = "scheduler.fileactionGoogleDrive.subject";
   public static final String PROPERTY_GOOGLEDRIVE_NOTIFICATION_TEXT_SUCCESS = "scheduler.fileactionGoogleDrive.text";

   private static final String PROPERTY_GOOGLEDRIVE_NOTIFICATION_DISABLED = "scheduler.fileactionGoogleDrive[@disabled]";
   private static final String PROPERTY_GOOGLEDRIVE_NOTIFICATION_HTML = "scheduler.fileactionGoogleDrive[@html]";
   ...
   @Inject
   public ScheduleAsGoogleDriveFileEmailNotificationHooker(@SchedulerModuleProperties Configuration config,
   ...
   public void actionExecutionEndedSuccessfully(AbstractJob abstractJob, AbstractAction abstractAction,
      ...
      if (!(abstractAction instanceof ScheduleAsGoogleDriveFileAction))
         return;

      ReportExecuteJob job = (ReportExecuteJob) abstractJob;

      try {
         sendmail((ScheduleAsGoogleDriveFileAction) abstractAction, job, null);
   ...
   private void sendmail(ScheduleAsGoogleDriveFileAction action, ReportExecuteJob job, Exception e)
         throws MessagingException {
      if (config.getBoolean(PROPERTY_GOOGLEDRIVE_NOTIFICATION_DISABLED, false))
         return;
      ...
      datamap.put(PROPERTY_OWNERS, UserListForJuelPrinter.createInstance(new ArrayList<>(job.getOwners()),
            config.getBoolean(PROPERTY_GOOGLEDRIVE_NOTIFICATION_HTML, false)));
      ...
      String subjectTemplate = config.getString(PROPERTY_GOOGLEDRIVE_NOTIFICATION_SUBJECT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionGoogleDriveMsgSubject"));
      String messageTemplate = config.getString(PROPERTY_GOOGLEDRIVE_NOTIFICATION_TEXT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionGoogleDriveMsgText"));

      MailTemplate mailTemplate = new MailTemplate();
      mailTemplate.setHtml(config.getBoolean(PROPERTY_GOOGLEDRIVE_NOTIFICATION_HTML, false));
