...
public class QuickExportDownloadServlet extends SecuredHttpServlet {
   ...
   @Inject
   public QuickExportDownloadServlet(Provider<SecurityService> securityServiceProvider,
         Provider<HttpExportService> httpExportServiceProvider, Provider<ZipUtilsService> zipServiceProvider,
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      String exName = new SimpleDateFormat("yyyyMMdd-hhmm").format(Calendar.getInstance().getTime()) + "-export-" + name
            + ".zip";

      response.setHeader(HttpUtils.CONTENT_DISPOSITION,
            httpUtilsProvider.get().makeContentDispositionHeader(true, exName));
