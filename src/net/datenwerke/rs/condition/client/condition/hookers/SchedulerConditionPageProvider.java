...
public class SchedulerConditionPageProvider implements ScheduleConfigWizardPageProviderHook {
   ...
   private ListStore<ScheduleConditionDto> store = new ListStore<ScheduleConditionDto>(
         new BasicObjectModelKeyProvider<ScheduleConditionDto>());
   ...
   protected void initPage(ReportDto report, ReportScheduleDefinition definition) {
      ...
      Scheduler.get().scheduleDeferred(() -> page.add(wrapper, new MarginData(10)));
   }
   ...
   private SimpleForm createForm(ReportScheduleDefinition definition) {
      ...
      failureStrategyKey = form.addField(List.class, ConditionMessages.INSTANCE.failureStrategyLabel(),
            new SFFCStaticDropdownList<ConditionFailureStrategy>() {
      ...
      retryStrategyUnitKey = form.addField(List.class, ConditionMessages.INSTANCE.failureRetryUnitLabel(),
            new SFFCStaticDropdownList<RetryTimeUnitDto>() {
      ...
      form.addCondition(failureStrategyKey, new FieldEquals(ConditionFailureStrategy.RETRY),
            new ShowHideFieldAction(retryStrategyUnitKey));
      form.addCondition(failureStrategyKey, new FieldEquals(ConditionFailureStrategy.RETRY),
            new ShowHideFieldAction(retryStrategyAmountKey));
   ...
   private Grid<ScheduleConditionDto> createGrid(ReportScheduleDefinition definition) {
      ...
      ColumnConfig<ScheduleConditionDto, String> ccName = new ColumnConfig<ScheduleConditionDto, String>(pa.name(), 200,
            BaseMessages.INSTANCE.propertyName());
      ccName.setMenuDisabled(true);
      columnConfig.add(ccName);

      ColumnConfig<ScheduleConditionDto, String> ccExpr = new ColumnConfig<ScheduleConditionDto, String>(
            pa.expression(), 200, ConditionMessages.INSTANCE.expressionLabel());
      ccExpr.setMenuDisabled(true);
      columnConfig.add(ccExpr);

      final Grid<ScheduleConditionDto> grid = new Grid<ScheduleConditionDto>(store,
            new ColumnModel<ScheduleConditionDto>(columnConfig));
      grid.setHeight(250);
   ...
   private ToolBar createToolbar(final ReportDto report) {
      ...
      addPredefinedConditionBtn = toolbarService
            .createSmallButtonLeft(ConditionMessages.INSTANCE.addPredefinedConditionLabel(), BaseIcon.ADD);
      ...
      addConditionBtn = toolbarService.createSmallButtonLeft(ConditionMessages.INSTANCE.addConditionLabel(),
            BaseIcon.ADD);
      ...
      removeAll.addSelectionHandler(new SelectionHandler<Item>() {
         ...
         public void onSelection(SelectionEvent<Item> event) {
            ConfirmMessageBox cmb = new DwConfirmMessageBox(ConditionMessages.INSTANCE.removeAllConfirmHeading(),
                  ConditionMessages.INSTANCE.removeAllConfirmText());
   ...
   protected void doShowAddCondition(ReportDto report, ListStore<Condition> store, final boolean expression) {
      ...
      ColumnConfig<Condition, Condition> ccName = new ColumnConfig<Condition, Condition>(
            new IdentityValueProvider<Condition>(), 150, BaseMessages.INSTANCE.propertyName());
      ccName.setMenuDisabled(true);
      ...
      ColumnConfig<Condition, Condition> ccDesc = new ColumnConfig<Condition, Condition>(
            new IdentityValueProvider<Condition>(), 200, BaseMessages.INSTANCE.propertyDescription());
      ...
      grid.addRowDoubleClickHandler(event -> onConditionSubmitButtonClick(grid, window, expression));
      ...
      cancelBtn.addSelectHandler(event -> window.hide());

      DwTextButton submitBtn = new DwTextButton(BaseMessages.INSTANCE.submit());
      window.addButton(submitBtn);
      submitBtn.addSelectHandler(event -> onConditionSubmitButtonClick(grid, window, expression));
   ...
   protected void displayEditScheduleCondition(final ScheduleConditionDto scheduleCondition) {
      ...
      final String taKey = form.addField(String.class, ScheduleConditionDtoPA.INSTANCE.expression(),
            new SFFCTextArea() {
      ...
      DwTextButton repBtn = toolbarService.createSmallButtonLeft(ConditionMessages.INSTANCE.addReplacementLabel(),
            BaseIcon.WRENCH);
      ...
      DwTextButton testBtn = toolbarService.createSmallButtonLeft(ConditionMessages.INSTANCE.testConditionLabel(),
            BaseIcon.COGS);
      tb.add(testBtn);
      testBtn.addSelectHandler(event -> {
         ...
         conditionDao.executeCondition(dummyCondition, new RsAsyncCallback<Boolean>() {
            ...
            public void onSuccess(Boolean result) {
               String msg = result ? ConditionMessages.INSTANCE.conditionHolds()
                     : ConditionMessages.INSTANCE.conditionFails();
      ...
      /* positioning bug */
      Scheduler.get().scheduleDeferred(() -> window.add(cwrapper, new MarginData(10)));
      window.show();
   }
   ...
   @Override
   public boolean isConfigured(ReportDto report, ReportScheduleDefinition definition) {
