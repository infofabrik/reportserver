 ...
 *
 */
public class ScheduleConditionConfigProvider implements ScheduleConfigProviderHook {
   ...
   @Inject
   public ScheduleConditionConfigProvider(DtoService dtoService, SecurityService securityService,
         ConditionService conditionService, HookHandlerService hookHandler) {
      ...
      if (null != list && list.getConditionList().size() > 0) {
         ...
         for (ScheduleConditionDto conditionDto : list.getConditionList()) {
            ...
            if (condition instanceof ReportConditionDto) {
               ...
               job.setProperty(ConditionModule.PROPERTY_PREFIX + i + ConditionModule.PROPERTY_POSTFIX_CONDITION_KEY,
                     ConditionModule.PROPERTY_POSTFIX_CONDITION_REPORT_COND_KEY);
               job.setProperty(ConditionModule.PROPERTY_PREFIX + i + ConditionModule.PROPERTY_POSTFIX_CONDITION_ID,
                     reportCondition.getId().toString());
               job.setProperty(ConditionModule.PROPERTY_PREFIX + i + ConditionModule.PROPERTY_POSTFIX_EXPRESSION,
                     conditionDto.getExpression());
   ...
   private void adaptScheduleDefinitionForCondition(ReportExecuteJob job, List<ScheduleConditionDto> list, int i) {
   ...
   private void adaptScheduleDefinitionForPredefinedCondition(ReportExecuteJob job, List<ScheduleConditionDto> list,
         int i) {
      if (job.hasProperty(ConditionModule.PROPERTY_PREFIX + i + ConditionModule.PROPERTY_POSTFIX_CONDITION_KEY)
            && !job.hasProperty(ConditionModule.PROPERTY_PREFIX + i + ConditionModule.PROPERTY_POSTFIX_CONDITION_ID)) {
         String key = job
