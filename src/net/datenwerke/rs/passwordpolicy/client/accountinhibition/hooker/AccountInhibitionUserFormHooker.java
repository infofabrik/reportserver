...
public class AccountInhibitionUserFormHooker extends SimpleFormViewHook {
   ...
   @Inject
   public AccountInhibitionUserFormHooker(AccountInhibitionServiceDao accountInhibitionDao,
   ...
   public void configureSimpleForm(final SimpleForm inhibitForm, final AbstractNodeDto selectedNode) {
      ...
      final String isInhibitedKey = inhibitForm.addField(Boolean.class,
            AccountInhibitionConfigurationPa.INSTANCE.inhibitionState(),
            SecurityMessages.INSTANCE.accountInhibitionLabel());

      final String expirationKey = inhibitForm.addField(Date.class,
            AccountInhibitionConfigurationPa.INSTANCE.expirationDate(),
            SecurityMessages.INSTANCE.accountExpirationDate(), new SFFCDate() {
      ...
      inhibitForm.addSubmissionCallback(new SimpleFormSubmissionCallback(inhibitForm) {
         ...
         public void formSubmitted() {
            ...
            accountInhibitionDao.applyAccountInhibitionConfiguration(config, new RsAsyncCallback<Void>() {
               @Override
      ...
      if (selectedNode instanceof UserDto) {
         accountInhibitionDao.getAccountInhibitionConfiguration((UserDto) selectedNode,
               new RsAsyncCallback<AccountInhibitionConfiguration>() {
                  @Override
