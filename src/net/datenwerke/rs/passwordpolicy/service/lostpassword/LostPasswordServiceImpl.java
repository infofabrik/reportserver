...
public class LostPasswordServiceImpl implements LostPasswordService {
   ...
   public String requestNewPassword(String username) throws ExpectedException {
      ...
      try {
         final User user = userManagerService.getUserByName(username);
         ...
         Objects.requireNonNull(user.getEmail(),
               "Your email address is empty. An administrator should correct this for being able to send you a new password per email.");
         ...
         if (null != salt)
            userPropertiesService.setPropertyValue(user, LostPasswordModule.USER_PROPERTY_TMP_PASSWORD,
                  passwordHasher.hashPassword(randomPassword, salt));
         else
            userPropertiesService.setPropertyValue(user, LostPasswordModule.USER_PROPERTY_TMP_PASSWORD,
                  passwordHasher.hashPassword(randomPassword));

         userPropertiesService.setPropertyValue(user, LostPasswordModule.USER_PROPERTY_TMP_PASSWORD_DATE,
