...
public class BsiPasswordPolicyUserMetadata {
   ...
   private void setUserPropertyIfChanged(UserPropertiesService userPropertiesService, User user, String propName,
         String newValue) {
   ...
   public void updateUser(User user, UserPropertiesService userPropertiesService) {
      if (null != lastSuccessfulLogin)
         setUserPropertyIfChanged(userPropertiesService, user, USER_PROPERTY_LAST_SUCCESSFUL_LOGIN_KEY,
               Long.toString(lastSuccessfulLogin.getTime()));
      ...
      if (null != lastFailedLogin)
         setUserPropertyIfChanged(userPropertiesService, user, USER_PROPERTY_LAST_FAILED_LOGIN_KEY,
               Long.toString(lastFailedLogin.getTime()));
      ...
      if (null != lastChangedPassword)
         setUserPropertyIfChanged(userPropertiesService, user, USER_PROPERTY_LAST_CHANGED_PASSWORD,
               Long.toString(lastChangedPassword.getTime()));
      ...
      if (failedLoginCount != -1)
         setUserPropertyIfChanged(userPropertiesService, user, USER_PROPERTY_FAILED_LOGIN_COUNTS,
      ...
      if (null != accountInhibited)
         setUserPropertyIfChanged(userPropertiesService, user, ReportServerModule.USER_PROPERTY_ACCOUNT_INHIBITED,
               Boolean.toString(accountInhibited));

      if (null != enforcePasswordChange)
         setUserPropertyIfChanged(userPropertiesService, user, USER_PROPERTY_MUST_CHANGE_PASSWD,
               Boolean.toString(enforcePasswordChange));

      if (null != accountExpirationDate)
         setUserPropertyIfChanged(userPropertiesService, user, ReportServerModule.USER_PROPERTY_ACCOUNT_EXPIRATION_DATE,
               Long.toString(accountExpirationDate.getTime()));
   ...
   public void loadfromUser(User user, UserPropertiesService userPropertiesService) {
      String lastSuccessfulLogin = userPropertiesService.getPropertyValue(user,
            USER_PROPERTY_LAST_SUCCESSFUL_LOGIN_KEY);
      ...
      String accountInhibited = userPropertiesService.getPropertyValue(user,
            ReportServerModule.USER_PROPERTY_ACCOUNT_INHIBITED);
      String enforcePasswordChange = userPropertiesService.getPropertyValue(user, USER_PROPERTY_MUST_CHANGE_PASSWD);
      String accountExporationDate = userPropertiesService.getPropertyValue(user,
            ReportServerModule.USER_PROPERTY_ACCOUNT_EXPIRATION_DATE);
   ...
   public boolean recentPasswordsContain(String cleartextPassword, int historyLength, PasswordHasher passwordHasher) {
      ...
      List<String> recent = recentPasswords.subList(Math.max(0, recentPasswords.size() - historyLength),
            recentPasswords.size());// .contains(password);
