 ...
 */
@Singleton
public class AccountInhibitionRpcServiceImpl extends SecuredRemoteServiceServlet
      implements AccountInhibitionRpcService {
   ...
   public AccountInhibitionConfiguration getAccountInhibitionConfiguration(@Named("user") UserDto user) {
      ...
      accountInhibitionModel
            .setInhibitionState(getInhibitionState(user).equals(InhibitionState.INHIBITED_ADMINISTRATIVELY));
      accountInhibitionModel
            .setBlockedTemporarily(getInhibitionState(user).equals(InhibitionState.BLOCKED_TEMPORARILY));
