...
public class UserVariableServiceImpl implements UserVariableService {
   ...
   @Inject
   public UserVariableServiceImpl(Provider<EntityManager> entityManagerProvider,
   ...
   @Override
   @SecurityChecked(genericTargetVerification = @GenericTargetVerification(target = UserVariableAdminViewSecurityTarget.class, verify = @RightsVerification(rights = Write.class)))
   ...
   public UserVariableInstance getVariableInstanceForUser(User user, UserVariableDefinition variableDefinition) {
      if (null == user) {
         throw new IllegalArgumentException("Failed retrieving value for UserVariable \"" + variableDefinition.getName()
               + "\": supplied User was null");
      }
