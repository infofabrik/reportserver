...
public class UserVariablesView extends MainPanelView {
   ...
   @Inject
   public UserVariablesView(ToolbarService toolbarService, UserVariablesUIService userVariableService,
   ...
   protected void createStoreSorter(ListStore<UserVariableInstanceDto> store) {
      store.addSortInfo(new StoreSortInfo<UserVariableInstanceDto>(new Comparator<UserVariableInstanceDto>() {
         ...
         @Override
         public int compare(UserVariableInstanceDto m1, UserVariableInstanceDto m2) {
   ...
   private void loadData() {
      /* load here variables */
      userVariableDao.getDefinedUserVariableInstances((AbstractUserManagerNodeDto) getSelectedNode(),
            new RsAsyncCallback<List<UserVariableInstanceDto>>() {
               @Override
      ...
      userVariableDao.getInheritedUserVariableInstances((AbstractUserManagerNodeDto) getSelectedNode(),
            new RsAsyncCallback<List<UserVariableInstanceDto>>() {
               @Override
               public void onSuccess(List<UserVariableInstanceDto> result) {
                  for (UserVariableInstanceDto instance : result)
      ...
      userVariableDao
            .getDefinedUserVariableDefinitions(new RsAsyncCallback<ListLoadResult<UserVariableDefinitionDto>>() {
               @Override
   ...
   private Grid<UserVariableInstanceDto> createGrid(ListStore<UserVariableInstanceDto> store, boolean inherited) {
      ...
      /* name column */
      ColumnConfig<UserVariableInstanceDto, UserVariableDefinitionDto> nameConfig = new ColumnConfig<UserVariableInstanceDto, UserVariableDefinitionDto>(
            uvInstPa.definition(), 200, BaseMessages.INSTANCE.name());
      nameConfig.setMenuDisabled(true);
      nameConfig.setCell(new AbstractCell<UserVariableDefinitionDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, UserVariableDefinitionDto value,
               SafeHtmlBuilder sb) {
      ...
      ColumnConfig<UserVariableInstanceDto, UserVariableInstanceDto> valueConfig = new ColumnConfig<UserVariableInstanceDto, UserVariableInstanceDto>(
            new IdentityValueProvider<UserVariableInstanceDto>("__value"), 200, BaseMessages.INSTANCE.value());
      valueConfig.setMenuDisabled(true);
      valueConfig.setCell(new AbstractCell<UserVariableInstanceDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, UserVariableInstanceDto value,
               SafeHtmlBuilder sb) {
            UserVariableConfigurator configurator = userVariablesService.getConfigurator(value);
            sb.appendEscaped(String.valueOf(
                  configurator.getDisplayValue(value, availableVariableDefinitions.findModel(value.getDefinition()))));
         }
      });
      columns.add(valueConfig);

      ColumnConfig<UserVariableInstanceDto, UserVariableDefinitionDto> descConfig = new ColumnConfig<UserVariableInstanceDto, UserVariableDefinitionDto>(
            uvInstPa.definition(), 200, BaseMessages.INSTANCE.description());
      ...
      if (inherited) {
         ColumnConfig<UserVariableInstanceDto, AbstractUserManagerNodeDto> inheritedConfig = new ColumnConfig<UserVariableInstanceDto, AbstractUserManagerNodeDto>(
               uvInstPa.folk(), 160, UserVariablesMessages.INSTANCE.definedAt());
         inheritedConfig.setMenuDisabled(true);
         inheritedConfig.setCell(new AbstractCell<AbstractUserManagerNodeDto>() {
            @Override
            public void render(com.google.gwt.cell.client.Cell.Context context, AbstractUserManagerNodeDto value,
                  SafeHtmlBuilder sb) {
               if (null != value)
      ...
      /* create grid */
      Grid<UserVariableInstanceDto> grid = new Grid<UserVariableInstanceDto>(store,
            new ColumnModel<UserVariableInstanceDto>(columns));
   ...
   private void addAddRemoveButtons(final Grid<UserVariableInstanceDto> grid, ToolBar toolbar) {
      ...
      addBtn.addSelectHandler(new SelectHandler() {
         ...
         public void onSelect(SelectEvent event) {
            ...
            SelectionPopup<UserVariableDefinitionDto> popup = new SelectionPopup<UserVariableDefinitionDto>(tmpStore,
                  displayProperties) {
               @Override
      ...
      removeAllButton.addSelectionHandler(new SelectionHandler<Item>() {
         ...
         public void onSelection(SelectionEvent<Item> event) {
            ConfirmMessageBox cmb = new DwConfirmMessageBox(UserVariablesMessages.INSTANCE.confirmRemoveAllHeader(),
                  UserVariablesMessages.INSTANCE.confirmRemoveText());
   ...
   protected void displayEditVariable(final UserVariableInstanceDto instance) {
      ...
      window.setHeading(UserVariablesMessages.INSTANCE.editVariable()
            + (instance.getDefinition().getName() == null ? BaseMessages.INSTANCE.unknown()
      ...
      Widget editComponent = config.getEditComponent(instance,
            availableVariableDefinitions.findModel(instance.getDefinition()));
   ...
   protected void defineVariables(List<UserVariableDefinitionDto> variable) {
      userVariableDao.addUserVariableInstances(variable, (AbstractUserManagerNodeDto) getSelectedNode(),
            new NotamCallback<List<UserVariableInstanceDto>>(UserVariablesMessages.INSTANCE.variableCreated()) {
               @Override
   ...
   protected void updateVariable(UserVariableInstanceDto instance) {
      userVariableDao.updateUserVariableInstance(instance,
            new NotamCallback<UserVariableInstanceDto>(BaseMessages.INSTANCE.changesApplied()) { // $NON-NLS-1$
               @Override
   ...
   protected void removeVariables(final List<UserVariableInstanceDto> instances) {
      userVariableDao.removeUserVariableInstances(instances,
            new NotamCallback<Void>(UserVariablesMessages.INSTANCE.removedVariable()) { // $NON-NLS-1$
               @Override
