...
public class UserVariablesDefinitionPanel extends MainPanelView {
   ...
   @Inject
   public UserVariablesDefinitionPanel(UserVariablesUIService userVariablesService, ToolbarService toolbarService,
   ...
   private Component createGrid() {
      ...
      /* name column */
      ColumnConfig<UserVariableDefinitionDto, String> nameConfig = new ColumnConfig<UserVariableDefinitionDto, String>(
            uvDefPa.name(), 200, BaseMessages.INSTANCE.name());
      nameConfig.setMenuDisabled(true);
      columns.add(nameConfig);

      /* desc column */
      ColumnConfig<UserVariableDefinitionDto, String> descConfig = new ColumnConfig<UserVariableDefinitionDto, String>(
            uvDefPa.description(), 400, BaseMessages.INSTANCE.propertyDescription());
      descConfig.setMenuDisabled(true);
      columns.add(descConfig);

      /* type column */
      ColumnConfig<UserVariableDefinitionDto, String> ccType = new ColumnConfig<UserVariableDefinitionDto, String>(
            new DisplayTitleValueProvider<UserVariableDefinitionDto>(), 200, BaseMessages.INSTANCE.type());
      ccType.setMenuDisabled(true);
      ccType.setSortable(false);
      columns.add(ccType);

      /* create grid */
      final Grid<UserVariableDefinitionDto> variableGrid = new Grid<UserVariableDefinitionDto>(variableStore,
            new ColumnModel<UserVariableDefinitionDto>(columns));
      ...
      editing.addCompleteEditHandler(new CompleteEditHandler<UserVariableDefinitionDto>() {
         @Override
         public void onCompleteEdit(CompleteEditEvent<UserVariableDefinitionDto> event) {
      ...
      removeAllButton.addSelectionHandler(new SelectionHandler<Item>() {
         ...
         public void onSelection(SelectionEvent<Item> event) {
            ConfirmMessageBox cmb = new DwConfirmMessageBox(UserVariablesMessages.INSTANCE.confirmRemoveAllHeader(),
                  UserVariablesMessages.INSTANCE.confirmRemoveAllText());
   ...
   protected void displayEditVariable(final UserVariableDefinitionDto definition) {
      ...
      window.setHeading(UserVariablesMessages.INSTANCE.editVariable()
            + (definition.getName() == null ? BaseMessages.INSTANCE.unnamed() : definition.getName()));
   ...
   protected void addVariable(UserVariableConfigurator config) {
      userVariableDao.addUserVariableDefinition(config.createDTOInstance(),
            new NotamCallback<UserVariableDefinitionDto>(UserVariablesMessages.INSTANCE.variableCreated()) {
               @Override
   ...
   protected void updateVariable(UserVariableDefinitionDto uvd) {
      userVariableDao.updateUserVariableDefinition(uvd,
            new NotamCallback<UserVariableDefinitionDto>(BaseMessages.INSTANCE.changesApplied()) {
               @Override
               public void doOnSuccess(UserVariableDefinitionDto result) {
   ...
   protected void removeVariables(final List<UserVariableDefinitionDto> definitions) {
      ...
      userVariableDao.removeUserVariableAndAskForForce(copy,
            new NotamCallback<Void>(UserVariablesMessages.INSTANCE.variableDeleted()) {
               @Override
