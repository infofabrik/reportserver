...
public class ScheduleAsSftpFileEmailNotificationHooker extends SchedulerExecutionHookAdapter {
   ...
   public static final String PROPERTY_SFTP_NOTIFICATION_SUBJECT_SUCCESS = "scheduler.fileactionSftp.subject";
   public static final String PROPERTY_SFTP_NOTIFICATION_TEXT_SUCCESS = "scheduler.fileactionSftp.text";

   private static final String PROPERTY_SFTP_NOTIFICATION_DISABLED = "scheduler.fileactionSftp[@disabled]";
   private static final String PROPERTY_SFTP_NOTIFICATION_HTML = "scheduler.fileactionSftp[@html]";
   ...
   @Inject
   public ScheduleAsSftpFileEmailNotificationHooker(@SchedulerModuleProperties Configuration config,
   ...
   public void actionExecutionEndedSuccessfully(AbstractJob abstractJob, AbstractAction abstractAction,
      ...
      if (!(abstractAction instanceof ScheduleAsSftpFileAction))
         return;

      ReportExecuteJob job = (ReportExecuteJob) abstractJob;

      try {
         sendmail((ScheduleAsSftpFileAction) abstractAction, job, null);
   ...
   private void sendmail(ScheduleAsSftpFileAction action, ReportExecuteJob job, Exception e) throws MessagingException {
      if (config.getBoolean(PROPERTY_SFTP_NOTIFICATION_DISABLED, false))
         return;
      ...
      datamap.put(PROPERTY_OWNERS, UserListForJuelPrinter.createInstance(new ArrayList<>(job.getOwners()),
            config.getBoolean(PROPERTY_SFTP_NOTIFICATION_HTML, false)));
      ...
      String subjectTemplate = config.getString(PROPERTY_SFTP_NOTIFICATION_SUBJECT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionSftpMsgSubject"));
      String messageTemplate = config.getString(PROPERTY_SFTP_NOTIFICATION_TEXT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionSftpMsgText"));

      MailTemplate mailTemplate = new MailTemplate();
      mailTemplate.setHtml(config.getBoolean(PROPERTY_SFTP_NOTIFICATION_HTML, false));
