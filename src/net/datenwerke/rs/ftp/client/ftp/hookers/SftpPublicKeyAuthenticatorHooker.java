...
public class SftpPublicKeyAuthenticatorHooker implements DatasinkAuthenticatorConfiguratorHook {
   ...
   public Widget configureForm(final SimpleFormView mainForm, final DatasinkDefinitionDto datasink) {
            ...
            DatasinksMessages.INSTANCE.privateKey(), new SFFCFileUpload() {
               ...
               public UploadProperties getProperties() {
                  UploadProperties uploadProperties = new UploadProperties("file",
                        FtpUiModule.SFTP_PRIVATE_KEY_UPLOAD_HANDLER_ID);
                  uploadProperties.addMetadata(FtpUiModule.SFTP_UPLOAD_DATASINK_ID_FIELD,
      ...
      /* passphrase */
      String passphraseKey = form.addField(String.class, SftpDatasinkDtoPA.INSTANCE.privateKeyPassphrase(),
            DatasinksMessages.INSTANCE.privateKeyPassphrase(), new SFFCPasswordField() {
               @Override
               public Boolean isPasswordSet() {
                  return ((SftpDatasinkDto) mainForm.getSelectedNode()).isHasPrivateKeyPassphrase();
               }
            }); // $NON-NLS-1$
      ...
      clearPassphraseMenu
            .addSelectionHandler(event -> ((SftpDatasinkDto) mainForm.getSelectedNode()).setPrivateKeyPassphrase(null));
