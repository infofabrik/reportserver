...
public class EditTeamSpaceMembersHooker extends TeamSpaceEditDialogHookImpl {
   ...
   public void setCurrentSpace(TeamSpaceDto teamSpace) {
      ...
      memberStore.addAll(teamSpace.getMembers()
         .stream()
         .filter(member -> member.getFolk().getId() != teamSpace.getOwner().getId())
         .map(member -> StrippedDownTeamSpaceMemberDto.createFrom(member))
         .collect(toList()));
   }
   ...
   private Widget createEditComponent() {
      ...
      addMemberBtn.addSelectHandler(event -> {
      ...
      addGroupBtn.addSelectHandler(event -> {
         Map<ValueProvider<StrippedDownGroup, String>, String> displayProperties = new LinkedHashMap<>();
   ...
   private Grid<StrippedDownTeamSpaceMemberDto> createMembersGrid() {
      ...
      editing.addCompleteEditHandler(event -> changed = true);
