...
public class TeamSpaceUIServiceImpl implements TeamSpaceUIService {
   ...
   @Inject
   public TeamSpaceUIServiceImpl(SecurityUIService securityService, TeamSpaceDao tsDao,
         DwMainViewportUiService viewportService, Provider<TeamSpaceClientMainModule> teamSpaceClientModuleProvider) {
   ...
   public ListLoader<ListLoadConfig, ListLoadResult<TeamSpaceDto>> getTeamSpacesLoader() {
      ...
      RpcProxy<ListLoadConfig, ListLoadResult<TeamSpaceDto>> proxy = new RpcProxy<ListLoadConfig, ListLoadResult<TeamSpaceDto>>() {
         @Override
         public void load(ListLoadConfig loadConfig, AsyncCallback<ListLoadResult<TeamSpaceDto>> callback) {
   ...
   public LoadableListStore<ListLoadConfig, TeamSpaceDto, ListLoadResult<TeamSpaceDto>> getTeamSpacesStore() {
      return new LoadableListStore<ListLoadConfig, TeamSpaceDto, ListLoadResult<TeamSpaceDto>>(tsPA.dtoId(),
            getTeamSpacesLoader());
   }
   ...
   public void displayAddSpaceDialog(final TeamSpaceOperationSuccessHandler successHandler) {
      ...
      form.addField(String.class, TeamSpaceDtoPA.INSTANCE.description(),
            TeamSpaceMessages.INSTANCE.newTeamSpaceDescriptionLabel(), new SFFCTextAreaImpl(80));
      ...
      submitBtn.addSelectHandler(new SelectHandler() {
         ...
         public void onSelect(SelectEvent event) {
            if (!form.isValid())
               return;
            window.hide();
            tsDao.createNewTeamSpace(dummy,
                  new NotamCallback<TeamSpaceDto>(TeamSpaceMessages.INSTANCE.newTeamSpaceCreatedMessage()) {
                     @Override
