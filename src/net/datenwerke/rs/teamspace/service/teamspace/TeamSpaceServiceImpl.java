...
public class TeamSpaceServiceImpl implements TeamSpaceService {
	...
	public void assertAccess(TeamSpace ts) {
		if(null == ts || ! mayAccess(ts))
			throw new ViolatedSecurityException("User does not have access to TeamSpace: " + (null == ts ? "" : ts.getId()));
	}
	
	@Override
	public void assertIsUser(TeamSpace teamSpace) {
		if(! isUser(teamSpace))
			throw new ViolatedSecurityException("User does not have User rights in TeamSpace: " + (null == teamSpace ? "" : teamSpace.getId()));
	}
