...
public class ScheduleAsAmazonS3FileEmailNotificationHooker extends SchedulerExecutionHookAdapter {
   ...
   public static final String PROPERTY_AMAZONS3_NOTIFICATION_SUBJECT_SUCCESS = "scheduler.fileactionAmazonS3.subject";
   public static final String PROPERTY_AMAZONS3_NOTIFICATION_TEXT_SUCCESS = "scheduler.fileactionAmazonS3.text";

   private static final String PROPERTY_AMAZONS3_NOTIFICATION_DISABLED = "scheduler.fileactionAmazonS3[@disabled]";
   private static final String PROPERTY_AMAZONS3_NOTIFICATION_HTML = "scheduler.fileactionAmazonS3[@html]";
   ...
   @Inject
   public ScheduleAsAmazonS3FileEmailNotificationHooker(@SchedulerModuleProperties Configuration config,
   ...
   public void actionExecutionEndedSuccessfully(AbstractJob abstractJob, AbstractAction abstractAction,
      ...
      if (!(abstractAction instanceof ScheduleAsAmazonS3FileAction))
         return;

      ReportExecuteJob job = (ReportExecuteJob) abstractJob;

      try {
         sendmail((ScheduleAsAmazonS3FileAction) abstractAction, job, null);
   ...
   private void sendmail(ScheduleAsAmazonS3FileAction action, ReportExecuteJob job, Exception e)
         throws MessagingException {
      if (config.getBoolean(PROPERTY_AMAZONS3_NOTIFICATION_DISABLED, false))
         return;
      ...
      datamap.put(PROPERTY_OWNERS, UserListForJuelPrinter.createInstance(new ArrayList<>(job.getOwners()),
            config.getBoolean(PROPERTY_AMAZONS3_NOTIFICATION_HTML, false)));
      ...
      String subjectTemplate = config.getString(PROPERTY_AMAZONS3_NOTIFICATION_SUBJECT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionAmazonS3MsgSubject"));
      String messageTemplate = config.getString(PROPERTY_AMAZONS3_NOTIFICATION_TEXT_SUCCESS,
            msgs.get(SchedulerMessages.class.getCanonicalName()).get("fileactionAmazonS3MsgText"));

      MailTemplate mailTemplate = new MailTemplate();
      mailTemplate.setHtml(config.getBoolean(PROPERTY_AMAZONS3_NOTIFICATION_HTML, false));
