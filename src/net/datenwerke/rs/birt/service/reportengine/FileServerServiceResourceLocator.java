...
public class FileServerServiceResourceLocator implements IResourceLocator {
   ...
   @Inject
   public FileServerServiceResourceLocator(FileServerService fileServerService, ConfigService configService,
   ...
   public URL findResource(String filename) {
      ...
      Long baseFolderId = configService.getConfigFailsafe(CONFIG_FILE)
            .getLong(BirtModule.BIRT_LIBRARY_BASE_FOLDER_ID_PROPERETY_NAME, null);
      if (null != baseFolderId)
         basefolder = fileServerService.getNodeById(baseFolderId);
      else {
         String baseFolderPath = configService.getConfigFailsafe(CONFIG_FILE)
               .getString(BirtModule.BIRT_LIBRARY_BASE_FOLDER_PATH_PROPERETY_NAME, null);
         if (null == baseFolderPath)
            throw new IllegalStateException("Could not find library base folder. Please set either property "
                  + BirtModule.BIRT_LIBRARY_BASE_FOLDER_ID_PROPERETY_NAME + " or "
                  + BirtModule.BIRT_LIBRARY_BASE_FOLDER_PATH_PROPERETY_NAME);
