...
public class RsTriggerFieldAppearance extends Css3TriggerFieldAppearance {
   ...
   public void render(SafeHtmlBuilder sb, String value, FieldAppearanceOptions options) {
      ...
      // outer div needed for widgets like comboBox that need the full width to set
      // for listview width
      String className = plainApperance ? "rs-trigger-plain" : "rs-trigger";
      sb.appendHtmlConstant(
            "<div class=\"" + className + "\" " + (plainApperance ? ">" : "style='width:" + width + "px;'>"));
      ...
      if (hideTrigger) {
         ...
         /*
          * The height/width of the trigger is generated based off the dimensions of the
          * image, which can negatively impact user experience on touch devices. For
          * touch devices, we're going to use the height of the input element to create a
          * large square around the trigger.
          */
         if (GXT.isTouch()) {
