...
public class RsButtonCellAppearance<M> extends Css3ButtonCellAppearance<M> {
   ...
   public void render(ButtonCell<M> cell, Context context, M value, SafeHtmlBuilder sb) {
      ...
      String buttonClass = style.button() + (scale == ButtonScale.LARGE ? " rs-btn-large" : "")
            + (iconAlign == IconAlign.TOP ? " rs-btn-top" : "");
      ...
      if (icon != null) {
         ...
         if (icon instanceof CssIconImageResource)
            iconBuilder.append(scale == ButtonScale.LARGE ? ((CssIconImageResource) icon).getCssIcon(1)
                  : ((CssIconImageResource) icon).getCssIcon());
         else
      ...
      // for left / right aligned icons with a fixed button width we render the icon
      // outside of the inner div
