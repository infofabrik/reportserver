...
public class MetaPoolService implements DbPoolService {
   ...
   @Inject
   public MetaPoolService(@PoolC3P0 DbPoolService c3p0, @UseConnectionPool Provider<Boolean> useConnectionPool,
   ...
   @Override
   public Future getConnection(ConnectionPoolConfig config) throws SQLException {
   ...
   @Override
   public Future getConnection(ConnectionPoolConfig poolConfig, ConnectionConfig connConfig) throws SQLException {
   ...
   protected Future<Connection> getConnectionDirectly(final ConnectionPoolConfig config,
      ...
      return new Future<Connection>() {
         ...
         public Connection get() throws InterruptedException, ExecutionException {
            ...
            /* create connection */
            try {
               conn = DriverManager.getConnection(jdbcService.adaptJdbcUrl(config.getJdbcUrl()), config.getUsername(),
                     config.getPassword());
   ...
   protected void configureConnection(Connection conn, ConnectionConfig connConfig) throws SQLException {
      ...
      if (null != connConfig.getIsolationLevel()
