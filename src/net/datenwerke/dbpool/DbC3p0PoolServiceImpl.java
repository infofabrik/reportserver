 ...
 *
 */
public class DbC3p0PoolServiceImpl extends DbPoolServiceImpl<ComboPooledDataSource>
   ...
   @Inject
   public DbC3p0PoolServiceImpl(EntityDiffService entityDiffService, ConfigService configService,
   ...
   @Override
   public Future<Connection> getConnection(ConnectionPoolConfig poolConfig, ConnectionConfig connConfig)
         throws SQLException {
   ...
   protected Future<Connection> getConnectionFromPool(final ConnectionPoolConfig poolConfig,
         final ConnectionConfig connConfig) throws SQLException {
      ReentrantLock lock;
      synchronized (DbC3p0PoolServiceImpl.class) {
         if (!lockMap.containsKey(poolConfig))
            lockMap.put(poolConfig, new ReentrantLock());
            ...
            public Connection get() throws InterruptedException, ExecutionException {
               ...
               try {
                  ...
                  try {
                     ...
                     useProxy = configService.getConfigFailsafe(DbPoolModule.CONFIG_FILE)
                           .getBoolean("pool.connection.proxy.enable", true);
                  } catch (Exception e) {
   ...
   private ComboPooledDataSource createPool(ConnectionPoolConfig config) throws SQLException {
      ...
      try {
         BeansUtils.overwriteAccessiblePropertiesFromMap(defaultUserProps, pool, false,
               Arrays.asList(new String[] { "loginTimeout", "properties" }), true, MLevel.FINEST, MLevel.WARNING,
               false);
   ...
   protected void configureConnection(Connection conn, ConnectionConfig connConfig) throws SQLException {
      ...
      if (null != connConfig && null != connConfig.getIsolationLevel()
            && metaData.supportsTransactionIsolationLevel(connConfig.getIsolationLevel()))
