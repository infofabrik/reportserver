...
public abstract class BasicACLView<T> extends VerticalLayoutContainer {
   ...
   @Inject
   public BasicACLView(@UserManagerTreeBasicSingleton UITree userManagerTree) {
   ...
   private void addAddRemoveButtons(final Grid<AceDto> grid, DwToolBar toolbar) {
      ...
      removeAllButton.addSelectionHandler(new SelectionHandler<Item>() {
         ...
         public void onSelection(SelectionEvent<Item> event) {
            ConfirmMessageBox cmb = new DwConfirmMessageBox(SecurityMessages.INSTANCE.confirmDeleteHeader(),
                  SecurityMessages.INSTANCE.confirmDeleteText());
   ...
   private void addRightsColumnConfigs(List<ColumnConfig<AceDto, ?>> columns, SecurityViewInformation information) {
      for (final SecureeDto securee : information.getAvailableSecurees()) {
         ...
         ColumnConfig<AceDto, String> ccSecuree = new ColumnConfig<AceDto, String>(new ValueProvider<AceDto, String>() {
            ...
            public String getValue(AceDto model) {
               ...
               AceAccessMapDtoDec accessMap = (AceAccessMapDtoDec) ((AceDtoDec) model)
   ...
   private ColumnConfig<AceDto, AceDto> createInheritanceColumnConfig() {
      ColumnConfig<AceDto, AceDto> ccInheritanceType = new ColumnConfig<AceDto, AceDto>(
            new IdentityValueProvider<AceDto>(), 120, SecurityMessages.INSTANCE.inheritance());
      ccInheritanceType.setCell(new AbstractCell<AceDto>() {

         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, AceDto value, SafeHtmlBuilder sb) {
   ...
   private ColumnConfig<AceDto, AceDto> createAccessTypeColumnConfig() {
      ColumnConfig<AceDto, AceDto> ccAccessType = new ColumnConfig<AceDto, AceDto>(new IdentityValueProvider<AceDto>(),
            120, SecurityMessages.INSTANCE.accessType());
      ccAccessType.setMenuDisabled(true);
      ccAccessType.setCell(new AbstractCell<AceDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, AceDto model, SafeHtmlBuilder sb) {
   ...
   private ColumnConfig<AceDto, AbstractUserManagerNodeDto> createFolkColumnConfig() {
      ColumnConfig<AceDto, AbstractUserManagerNodeDto> ccFolk = new ColumnConfig<AceDto, AbstractUserManagerNodeDto>(
            acePa.folk(), 200, SecurityMessages.INSTANCE.folk());
      ccFolk.setMenuDisabled(true);
      ccFolk.setCell(new AbstractCell<AbstractUserManagerNodeDto>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, AbstractUserManagerNodeDto folk,
               SafeHtmlBuilder sb) {
            if (null == folk) {
   ...
   protected void displayEditDialog(final AceDto selectedACE, final SecurityViewInformation information) {
      ...
      /* folk */
      form.addField(AbstractUserManagerNodeDto.class, AceDto.PROPERTY_FOLK, SecurityMessages.INSTANCE.folk(),
            selectedACE.getFolk(), new SFFCGenericTreeNode() {
      ...
      /* access type */
      form.addField(List.class, AceDto.PROPERTY_ACCESSTYPE, SecurityMessages.INSTANCE.accessType(),
      ...
      if (selectedACE instanceof HierarchicalAceDto) {
         form.addField(List.class, HierarchicalAceDto.PROPERTY_INHERITANCETYPE, SecurityMessages.INSTANCE.inheritance(),
               ((HierarchicalAceDto) selectedACE).getInheritancetype(), new SFFCStaticRadioList<InheritanceTypeDto>() {
                  @Override
      ...
      /* add type */
      final String fastAccessKey = form.addField(List.class, "fastAccess", SecurityMessages.INSTANCE.fastAccess(), //$NON-NLS-1$ //$NON-NLS-2$
      ...
      for (SecureeDto securee : information.getAvailableSecurees()) {
         ...
         for (RightDto right : securee.getRights()) {
            ...
            /* add field */
            form.addField(Boolean.class, rightId, right.getDescription() + " (" + right.getAbbreviation() + ")", //$NON-NLS-1$ //$NON-NLS-2$
   ...
   protected void editACE(AceDto selectedACE, SimpleForm form, SecurityViewInformation information) {
      ...
      if (selectedACE instanceof HierarchicalAceDto)
         ((HierarchicalAceDto) selectedACE)
