...
public class SecurityUIStartup {
	...
	public SecurityUIStartup(
		...
		waitOnEventService.callbackOnEvent(LoginService.REPORTSERVER_EVENT_AFTER_INITIAL_LOGIN, (final WaitOnEventTicket ticket) -> {
			final Collection<GenericTargetIdentifier> targetIdentifiers = new HashSet<>();
			
			targetIdentifiers.addAll(
					hookHandler.getHookers(GenericTargetProviderHook.class)
						.stream()
						.map(GenericTargetProviderHook::getObject)
						.collect(toList()));
