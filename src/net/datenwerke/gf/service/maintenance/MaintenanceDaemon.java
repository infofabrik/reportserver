...
public class MaintenanceDaemon extends DwDaemonImpl {
   ...
   @Inject
   public MaintenanceDaemon(Provider<EntityManager> entityManagerProvider, Provider<UnitOfWork> unitOfWorkProvider,

         Provider<HookHandlerService> hookHandlerProvider, Provider<EventBus> eventLoggerProvider,
   ...
   protected void doWork() {
      for (MaintenanceTask task : hookHandlerProvider.get().getHookers(MaintenanceTask.class)) {
         ...
         try {
            ...
            try {
               eventLoggerProvider.get().fireEvent(new MaintenanceJobStarted("name", task.getClass().getName(), "time",
               ...
               eventLoggerProvider.get().fireEvent(new MaintenanceJobEnded("name", task.getClass().getName(), "time",
                     String.valueOf(new Date().getTime())));
            } catch (Exception e) {
               eventLoggerProvider.get()
                     .fireEvent(new MaintenanceJobFailed("name", task.getClass().getName(), "error", e.getMessage()));
   ...
   protected long getNextSleepTime() {
      ...
      try {
         ...
         em = entityManagerProvider.get();
         em.getTransaction().begin();
         return configService.getConfigFailsafe(MaintenanceModule.CONFIG_FILE).getLong("maintenance.tasks.interval",
               60000l);
      } finally {
         try {
            em.getTransaction().rollback();
