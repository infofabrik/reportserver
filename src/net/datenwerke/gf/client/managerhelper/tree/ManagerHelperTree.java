...
public class ManagerHelperTree extends UITree {
   ...
   @Inject
   public ManagerHelperTree(ClipboardUiService clipboardService, HookHandlerService hookHandler,
         Provider<TreeDBUIMenuProvider> menuConfigProvider, @Assisted Class<?> guarantor,
         @Assisted EnhancedTreeStore store, @Assisted TreeDbLoaderDao treeLoaderDao,
                        ...
                        if (doubleClicked > 0)
                           ...
                           treeLoaderDao.loadFullViewNode(node, new RsAsyncCallback<AbstractNodeDto>() {
                              ...
                              public void onSuccess(AbstractNodeDto fullNode) {
                                 ...
                                 Collection<MainPanelViewProviderHook> viewProviders = hookHandler
                                       .getHookers(MainPanelViewProviderHook.class);
                                 final List<MainPanelView> views = new ArrayList<MainPanelView>();
                                 for (MainPanelViewProviderHook viewProvider : viewProviders) {
                                    List<MainPanelView> newViews = viewProvider
                                          .mainPanelViewProviderHook_getView(fullNode);
                                    if (null != newViews)
                                 ...
                                 TreePostSelectAsyncHook me = new TreePostSelectAsyncHook() {
                                    @Override
                                    public void postprocessNode(AbstractNodeDto selectedNode,
                                          List<TreePostSelectAsyncHook> next) {
                                       if (!views.isEmpty()) {
                                 ...
                                 List<TreePostSelectAsyncHook> postSelectHookers = hookHandler
                                       .getHookers(TreePostSelectAsyncHook.class);
                                 ...
                                 postSelectHookers.get(0).postprocessNode(fullNode,
                                       postSelectHookers.subList(1, postSelectHookers.size()));
   ...
   protected void updateHistoryOnSelection(AbstractNodeDto fullNode) {
