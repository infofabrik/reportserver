...
public class FileSelectionWidget extends VerticalLayoutContainer {
   ...
   @Inject
   public FileSelectionWidget(FileSelectionDao fileSelectionDao, FileDownloadUiService downloadService) {
      super();
   ...
   protected void buildGrid() {
      ...
      ColumnConfig<SelectedFileWrapper, SelectedFileWrapper> iconConfig = new ColumnConfig<SelectedFileWrapper, SelectedFileWrapper>(
            new IdentityValueProvider<SelectedFileWrapper>("__file_icon"), 30);
      iconConfig.setMenuDisabled(true);
      iconConfig.setCell(new AbstractCell<SelectedFileWrapper>() {
         @Override
         public void render(com.google.gwt.cell.client.Cell.Context context, SelectedFileWrapper value,
               SafeHtmlBuilder sb) {
      ...
      /* Name column */
      ColumnConfig<SelectedFileWrapper, String> nameConfig = new ColumnConfig<SelectedFileWrapper, String>(
            SelectedFileWrapperPA.INSTANCE.name(), 250, BaseMessages.INSTANCE.name());
      nameConfig.setMenuDisabled(true);
      columns.add(nameConfig);

      /* Type column */
      ColumnConfig<SelectedFileWrapper, SelectedFileWrapper> typeConfig = new ColumnConfig<SelectedFileWrapper, SelectedFileWrapper>(
            new IdentityValueProvider<SelectedFileWrapper>("__file_type"), 200, BaseMessages.INSTANCE.type());
      ...
      editing.addBeforeStartEditHandler(new BeforeStartEditHandler<SelectedFileWrapper>() {
         @Override
         public void onBeforeStartEdit(BeforeStartEditEvent<SelectedFileWrapper> event) {
   ...
   public void displayMaxFilesReachedMessage() {
      new DwAlertMessageBox(FileSelectionMessages.INSTANCE.maxFilesReachedTitle(),
            FileSelectionMessages.INSTANCE.maxFilesReachedMessage(maxFiles)).show();
   }
}
