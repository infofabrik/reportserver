...
public class FieldInfoHandler implements HasClickHandlers, ErrorHandler {
   ...
   public void mark(List<String> messages) {
      ...
      if (tip == null) {
         tip = new ToolTip(infoIcon.getWidget(),
               GWT.<SideErrorTooltipAppearance>create(SideErrorTooltipAppearance.class));
      }
   ...
   /**
    * Adjusts the size of the widget to account for the side icon, if visible. May
    * be called multiple times, whether or not an error is displayed, and must be
    * called again when size changes to account for that change.
    */
   ...
   /**
    * Restores the widget to its original size. Need only be called when in the
    * process of getting rid of an error, when showingError is still true.
    */
   ...
   /**
    * True to adjust the target width when an error is displayed (defaults to
    * true).
    * 
   ...
   protected void alignInfoIcon() {
      Scheduler.get().scheduleDeferred(new ScheduledCommand() {
         ...
         public void execute() {
            ...
            infoIcon.getElement().alignTo(input, new AnchorAlignment(Anchor.TOP_LEFT, Anchor.TOP_RIGHT, false), 2, 3);
         }
      });
   }
