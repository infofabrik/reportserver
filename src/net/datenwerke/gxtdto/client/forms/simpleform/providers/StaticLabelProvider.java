...
public class StaticLabelProvider implements FormFieldProviderHook {
	...
	private SFFCStaticLabel getSFFCStaticLabel(SimpleFormFieldConfiguration[] configArray){
		return Arrays
				.stream(configArray)
				.filter(config -> config instanceof SFFCStaticLabel)
				.map(config -> (SFFCStaticLabel) config)
				.findAny()
				.orElse(null);
	}
	...
	private SFFCStaticHtml getSFFCStaticHtml(SimpleFormFieldConfiguration[] configArray) {
		return Arrays
				.stream(configArray)
				.filter(config -> config instanceof SFFCStaticHtml)
				.map(config -> (SFFCStaticHtml) config)
				.findAny()
				.orElse(null);
	}
	...
	private SFFCStaticTextField getSFFCStaticTextField(SimpleFormFieldConfiguration[] configArray){
		return Arrays
				.stream(configArray)
				.filter(config -> config instanceof SFFCStaticTextField)
				.map(config -> (SFFCStaticTextField) config)
				.findAny()
				.orElse(null);
	}
