...
public class StringProvider extends FormFieldProviderHookImpl {
   ...
   public Widget createFormField() {
      ...
      if (null != textAreaConfig && (textAreaConfig.getHeight() > 1 || textAreaConfig.getHeight() == -1)) {
         ...
         if (null != codeMirrorConfig) {
            field = new CodeMirrorPanel(
                  new CodeMirrorConfig(codeMirrorConfig.getLanguage(), codeMirrorConfig.lineNumbersVisible()),
                  codeMirrorConfig.getEnhancer());
   ...
   protected void addChangeListener(Widget field) {
      if (field instanceof HasValueChangeHandlers) {
         ((HasValueChangeHandlers) field)
               .addValueChangeHandler(event -> ValueChangeEvent.fire(StringProvider.this, event.getValue()));
      }
   }
   ...
   protected boolean isDecodeHtml() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCNoHtmlDecode).map(config -> Boolean.FALSE)
            .findAny().orElseGet(() -> Boolean.TRUE);
   }

   protected SFFCCodeMirror getCodeMirrorConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCCodeMirror)
            .map(config -> (SFFCCodeMirror) config).findAny().orElse(null);
   }

   protected SFFCRichTextEditor getRichTextEditorConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCRichTextEditor)
            .map(config -> (SFFCRichTextEditor) config).findAny().orElse(null);
   }

   protected boolean decodeHtml() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCStringNoHtmlDecode)
            .map(config -> Boolean.FALSE).findAny().orElseGet(() -> Boolean.TRUE);
   }

   protected SFFCStringValidator getStringValidatorConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCStringValidator)
            .map(config -> (SFFCStringValidator) config).findAny().orElse(null);
   }

   protected SFFCPasswordField getPasswordConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCPasswordField)
            .map(config -> (SFFCPasswordField) config).findAny().orElse(null);
   }

   protected SFFCTextArea getTextareaConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCTextArea)
            .map(config -> (SFFCTextArea) config).findAny().orElse(null);
   }

   protected SFFCStringMaxLength getMaxLengthConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCStringMaxLength)
            .map(config -> (SFFCStringMaxLength) config).findAny().orElse(null);
   }
   ...
   protected SFFCReadOnly getReadOnlyConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCReadOnly)
            .map(config -> (SFFCReadOnly) config).findAny().orElse(null);
   }

   protected SFFCPlaceHolder getPlaceHolderConfig() {
      return Arrays.stream(configs).filter(config -> config instanceof SFFCPlaceHolder)
            .map(config -> (SFFCPlaceHolder) config).findAny().orElse(null);
   }
