...
public abstract class ImporterImpl<C extends ImportItemConfig> implements Importer {
   ...
   @Override
   public void configure(ImportSupervisor supervisor, Collection<ImporterSpecificConfig> specificConfigs,
         Queue<ImportItemConfig> itemConfigs) {
   ...
   public void importReference(ImportItemConfig itemConfig) {
      if (!itemConfigs.contains(itemConfig))
         throw new ImportException(
               "Item has already been imported or a terrible error (probably result has not been registered) occured. Id: "
                     + itemConfig.getId());
