...
public abstract class AbstractNode<N extends AbstractNode<N>> implements Serializable {
   ...
   /**
    * Retrieves all children nodes of this node, also transitively. I.e. if there
    * are any child folders, also all their children nodes are retrieved.
    * 
    ...
    * transitively. I.e. if there are any child folders, also all their children
    * nodes are retrieved.
    * 
   ...
   public <T extends AbstractNode<N>> List<T> getDescendants(Class<T> type) {
      return getDescendants().stream().filter(descendant -> type.isInstance(descendant))
            .map(descendant -> type.cast(descendant)).collect(toList());
   }
   ...
   /**
    * Add a new childnode to this node. Childnode will become detached from old
   ...
   public void addFlag(long bitfield) {
      if (wouldContainFlag(FLAG_CONFIGURATION_PROTECTION, this.flags | bitfield)
            && !wouldContainFlag(FLAG_WRITE_PROTECTION, this.flags | bitfield)) {
      ...
      if (hasFlag(bitfield)) {
         if (wouldContainFlag(FLAG_CONFIGURATION_PROTECTION, this.flags ^ bitfield)
            ...
            throw new IllegalArgumentException(
                  "Not allowed: Report would be config protected, but not write protected");
         }
