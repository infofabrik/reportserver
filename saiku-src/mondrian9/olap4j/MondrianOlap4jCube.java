...
{
    final mondrian9.olap.Cube cube;
    ...
    public NamedList<Dimension> getDimensions() {
        ...
        final mondrian9.olap.SchemaReader schemaReader =
            olap4jConnection.getMondrianConnection2().getSchemaReader()
            .withLocus();
        for (mondrian9.olap.Dimension dimension
    ...
    public NamedList<Hierarchy> getHierarchies() {
        ...
        for (mondrian9.olap.Dimension dimension
            : schemaReader.getCubeDimensions(cube))
        {
            for (mondrian9.olap.Hierarchy hierarchy
    ...
    public List<Measure> getMeasures() {
        ...
        try {
            ...
            List<mondrian9.olap.Member> levelMembers =
                schemaReader.getLevelMembers(
                    measuresLevel.level,
                    true);
            for (mondrian9.olap.Member member : levelMembers) {
                ...
                // mondrian9.olap4j.MondrianOlap4jMember cannot be cast to
    ...
    public NamedList<NamedSet> getSets() {
        ...
        final MondrianOlap4jConnection olap4jConnection =
            olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData.olap4jConnection;
        for (mondrian9.olap.NamedSet namedSet : cube.getNamedSets()) {
    ...
    private MondrianOlap4jMember lookupMember(
        ...
        List<IdentifierSegment> nameParts)
    {
        final List<mondrian9.olap.Id.Segment> segmentList =
            new ArrayList<mondrian9.olap.Id.Segment>();
        for (IdentifierSegment namePart : nameParts) {
            segmentList.add(Util.convert(namePart));
        }
        final mondrian9.olap.Member member =
