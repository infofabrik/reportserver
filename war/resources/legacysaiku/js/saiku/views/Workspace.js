...
var Workspace = Backbone.View.extend({
    ...
    initialize: function(args) {
        ...
        if (args && args.query) {
            ...
            var ws = this;
            this.query.save({}, { success: this.init_query, error: function(jqXHR, textStatus, errorThrown){ ws.error({"data" : {"error" : textStatus.statusText + "\n" + textStatus.responseText}}); } });
        }
...
//            .animate({ backgroundColor: '#fff' }, 'slow');
    ...
    new_query: function() {
        ...
        var ws = this;
        this.query.save({},{ async: false, error: function(jqXHR, textStatus, errorThrown){ ws.error({"data" : {"error" : textStatus.statusText + "\n" + textStatus.responseText}}); } });
        this.init_query();
    },
    ...
    error: function(args) {
    	var msg = args.data.error;
    	if(null == msg)
    		return;
    	
    	if(msg.indexOf("[[XX_RS_ERROR]]") !== -1){
    		msg = msg.substring(msg.indexOf("[[XX_RS_ERROR]]")+15, msg.indexOf("[[/XX_RS_ERROR]]")) 
    	}
        this.processing.html(safe_tags_replace(msg)).show();
    }
});
